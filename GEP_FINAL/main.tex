...
\begin{itemize}
    \item \textbf{Definición de Roles de Agentes:} Diseñar e implementar perfiles lógicos con responsabilidades estrictamente separadas (agente diseñador, agente desarrollador, agente revisor) para simular un ciclo de desarrollo de software completo.
    \item \textbf{Gestión de Estado y Contexto:} Implementar un sistema de memoria centralizado y persistente dentro del repositorio del proyecto, capaz de almacenar reglas arquitectónicas, el plan de ruta (\textit{roadmap}) y el estado actual de las tareas.
    \item \textbf{Desarrollo de Capacidades (\textit{Agent Skills}):} Dotar a los agentes de herramientas que les permitan interactuar con el entorno del proyecto (lectura y escritura de archivos, análisis de la estructura del repositorio, etc.).
    \item \textbf{Implementación del Bucle Iterativo:} Establecer y probar un flujo de trabajo validado que vaya desde la concepción de la funcionalidad hasta la implementación y revisión final del código generado.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Limitaciones del Ecosistema IDE:} La integración profunda con los editores de código a través de protocolos recientes puede presentar inestabilidades tecnológicas o limitar la portabilidad inicial del sistema a un IDE específico.
    \item \textbf{Gestión de la Ventana de Contexto:} A medida que el proyecto crezca, mantener todo el conocimiento arquitectónico en la memoria del agente sin exceder los límites de \textit{tokens} del LLM supondrá un reto técnico significativo.
    \item \textbf{Fricción en el Bucle de Desarrollo:} Existe el riesgo de que las políticas de auditoría estricta bloqueen constantemente al usuario. Será crucial calibrar la tolerancia del sistema ante desviaciones menores para no perjudicar la experiencia del desarrollador.
\end{itemize}
...
El control se basará en comparar los valores estimados en este documento con los gastos y horas reales consumidas. Para cuantificar estas posibles diferencias, se definen los siguientes indicadores numéricos de control:
...
\begin{itemize}
    ...
    \item \textbf{Consumo del fondo de imprevistos:} Evalúa cuánto remanente nos queda del fondo de emergencias calculado. Si el resultado es negativo, significa que hemos agotado el presupuesto de seguridad.
    \begin{equation}
        Estado\ Imprevistos = Total\ Imprevistos\ Estimado - Coste\ Imprevistos\ Consumido
    \end{equation}
\end{itemize}
Si durante una revisión de \textit{Sprint} se detecta que los indicadores arrojan valores negativos sostenidos (es decir, el coste real supera al estimado), se tomarán medidas correctivas, tales como reajustar el \textit{Sprint Backlog}, priorizar tareas críticas o, en última instancia, reducir el alcance (\textit{scope downgrade}) de funciones secundarias para no comprometer la entrega final del proyecto.
