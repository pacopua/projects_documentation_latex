...
\begin{titlepage}
    ...
    {\Huge \textsc{\textbf{}}\par}
    \vspace{2cm}
    {\huge \textbf{S{\LARGE ISTEMA} I{\LARGE NTELIGENTE} {\LARGE DE} A{\LARGE SISTENCIA} {\LARGE AL} D{\LARGE ESARROLLO} E{\LARGE STRUCTURADO} {\LARGE Y} S{\LARGE EGUIMIENTO} {\LARGE DE} P{\LARGE ROYECTOS} {\LARGE DE} S{\LARGE OFTWARE}}\par} % Esto es ridículisimo, pero me gusta como se ven las letras, escribo aquí el texto por si en algún momento queremos hacer ctrl + f poder encontrarlo "SISTEMA INTELIGENTE DE ASISTENCIA AL DESARROLLO ESTRUCTURADO Y SEGUIMIENTO DE PROYECTOS DE SOFTWARE"
    \vspace{2cm}
...
El presente trabajo consiste en el diseño de un sistema de automatización para el desarrollo y diseño de proyectos de software, utilizando como base herramientas basadas en \textit{LLMs}. El desarrollo de este sistema se enmarca y diseña para su uso dentro de una empresa real, \textit{Colb.ai}, dedicada principalmente a la consultoría de proyectos de software.

Es innegable que las herramientas de inteligencia artificial, principalmente los grandes modelos de lenguaje (\textit{LLMs}), están cambiando la forma en que se desarrollan los proyectos de software en la mayor parte de las empresas del sector. Estas tecnologías presentan la oportunidad de automatizar muchas de las tareas que tradicionalmente habían sido llevadas a cabo exclusivamente por los desarrolladores humanos. Sin embargo, a pesar de que habitualmente consigan resultados muy rápidos al comienzo del proyecto, la implementación sin un plan premeditado de estas herramientas sin una correcta supervisión humana, conlleva consecuencias como deudas técnicas, código no escalable o problemas de seguridad y eficiencia en el software a largo plazo.

Lo que se busca en concreto con este proyecto, es ahorrar tiempo y recursos de la empresa, automatizando al máximo las principales etapas contenidas en el ciclo de vida del desarrollo de un proyecto software. Pero evitando caer en los problemas mencionados, mediante la planificación y guía de los procesos.

Se presentan tres apartados que se van a automatizar en el desarrollo de este TFG:
\begin{enumerate}
...
El principal problema que se presenta en la automatización de estas tareas, sin perder calidad, es conseguir un equilibrio entre la agilidad que ofrecen los asistentes de inteligencia artificial, y la involucración de un desarrollador que garantice que cada paso cubre con unos estándares de estructura y calidad.

Este equilibrio es difícil de mantener en la práctica, y su dificultad puede explicarse a través de un principio en el ámbito de la psicología cognitiva: \textit{The Law of Least Effort}, o en otras palabras, la ley del mínimo esfuerzo. Esta teoría sostiene que los humanos y otros organismos tienden a priorizar la opción que presenta la menor resistencia para alcanzar sus objetivos, ya sea en términos de esfuerzo físico o cognitivo \cite{inzlicht_effort_paradox_2018}. Aunque pueda parecer paradójico -puesto que a las personas también nos gustan los retos y los puzles-, este fenómeno explica con precisión la principal trampa de la automatización: el desarrollador tiene la tentación de delegar todo el trabajo en el asistente de IA sin involucrarse lo suficiente, comprometiendo así los estándares de calidad y estructura necesarios para un proyecto profesional y sostenible a largo plazo.

A continuación, se detallan los problemas específicos identificados en cada apartado.

\subsubsection{Toma de requerimientos y planificación de la arquitectura}
\label{sec:ProblemaTomaRequerimientos}

La toma de requerimientos es una fase especialmente crítica en el desarrollo de software, puesto que sienta las bases de lo que va a ser el desarrollo del proyecto, el alcance de este, las técnologia a utilizar, y la arquitectura del sistema. Si esta fase no se lleva a cabo con el adecuado rigor, puede traer consecuencias muy graves para el resto del desarrollo, por lo que es extremadamente importante que si se quiere automatizar esta fase, se haga de una forma que garantice que el resultado es un documento de especificación completo, coherente y estructurado, que sirva como base sólida para la implementación posterior.
...
Este apartado se considera especialmente relevante en lo que es el ámbito de la consultoría, debido a que la relación y involucramiento con el cliente es una componente esencial para conseguir que un proyecto de software progrese adecuadamente. En este sentido, la automatización de esta fase tiene el riesgo de que la información que se transmite al cliente no sea clara, o directamente sea incorrecta.
...
\subsection{Stakeholders} % Esta sección debe cambiar un poco el enfoque, debido al cambio que ha habido, donde ahora también se cubre la toma de requerimientos y el seguimiento del cliente, no solo el desarrollo de código.
...
\begin{description}
    \item[\textbf{Toma de Requerimientos:}] Diseñar e implementar un flujo de trabajo asistido por IA que, a partir de la transcripción de una reunión con el cliente, sea capaz de generar un plan de implementación estructurado, explorando automáticamente los frameworks y tecnologías relevantes mediante búsqueda semántica en la web. Implementación documentada en el apartado \ref{sec:toma_requerimientos_cap}.
    \item[\textbf{Spec Driven Development (Estructura del repositorio):}] Integrar y configurar los asistentes de código de tipo CLI (ClaudeCode y OpenCode) con un conjunto de herramientas, plugins y workflows personalizados que impongan un flujo de trabajo Spec-First al equipo de desarrollo, garantizando que cada implementación esté precedida por una especificación validada y revisada. Documentado en la sección \ref{sec:sdd_implementacion}.
    \item[\textbf{Seguimiento del cliente:}] Automatizar la generación de documentación de seguimiento post-reunión, produciendo de forma estructurada el resumen de lo acordado y los próximos pasos para ambas partes, a partir de la transcripción de las reuniones periódicas con el cliente. Documentado en la sección \ref{sec:seguimiento_cliente}.
\end{description}

\subsection{Actualización del estado del arte y tecnologías adoptadas}
\label{sec:estado_arte_actualizado}

El cambio de alcance introduce dos pilares nuevos ---toma de requerimientos y seguimiento del cliente--- que no estaban presentes en el análisis de mercado del GEP inicial. A continuación se actualiza el estado del arte para los tres pilares del proyecto.

\subsubsection{Toma de requerimientos.} 
Las plataformas de videoconferencia (Microsoft Teams, Google Meet, Zoom) ofrecen 
transcripción automática integrada para reuniones telemáticas, por lo que no requieren solución propia. Para reuniones presenciales, las principales alternativas son Whisper \cite{radford2022whisper} (OpenAI, ejecución local, sin coste de API pero sin diarización nativa) y Azure Speech Service (cloud, con diarización de hablantes). Se optó por Azure Speech Service al estar incluido en la suscripción de Azure de Colb.AI y por ofrecer resultados satisfactorios en las pruebas realizadas. Para la conversión de transcripciones en especificaciones técnicas, la solución principal estudiada fue Claude Cowork mediante skills, que permite definir flujos de trabajo reutilizables con acceso a herramientas de búsqueda web y escritura de archivos.

Como alternativa open source se evaluó OpenWork 
\cite{differentai2025openwork}, una aplicación de escritorio 
construida sobre OpenCode que replica las capacidades de 
Claude Cowork; sin embargo, su versión de escritorio para 
Windows requiere el plan de pago y la comunidad reporta 
inestabilidad significativa. Ante esta limitación, se desarrolló 
una implementación propia mediante una aplicación Python 
orquestada con LangChain, que replica el flujo de Claude Cowork 
de forma independiente de cualquier suscripción: incorpora un 
pipeline de chunking para procesar transcripciones largas, 
búsqueda semántica web mediante Tavily para investigar las 
tecnologías identificadas, y una estrategia de dos niveles de 
modelo que combina Azure OpenAI para el filtrado de alto 
volumen con Google Gemini para la síntesis arquitectónica. 
Esta implementación se describe en detalle en la 
sección \ref{sec:toma_requerimientos_cap}.

\subsubsection{Spec Driven Development.} 
El análisis del GEP inicial ---Spec Kit, AgentOS, Copilot y Cursor--- sigue siendo válido 
para caracterizar el problema general. En cuanto a asistentes 
extensibles, las tres herramientas principales del ecosistema 
---ClaudeCode \cite{claude_code_2024}, OpenCode \cite{opencode_2025} y 
Cursor \cite{cursor}--- exponen mecanismos equivalentes de 
extensibilidad: hooks de ciclo de vida, protocolo MCP, skills y 
agentes. El plugin colbPowers está diseñado sobre estos 
estándares comunes y es en gran medida compatible con los tres 
asistentes. Se optó por ClaudeCode como herramienta principal 
al estar incluida en la suscripción de Colb.AI, y por OpenCode 
como alternativa open source para mitigar el riesgo de 
dependencia de proveedor, tal como se describe en la 
sección \ref{sec:sdd_implementacion}.

\subsubsection{Seguimiento del cliente.} 
Existen herramientas 
comerciales de transcripción y resumen de reuniones (Otter.ai, 
Fireflies.ai, Microsoft Copilot for Teams) que generan 
documentación post-reunión de forma automática. Sin embargo, 
independientemente de su grado de personalización, la tarea de 
seguimiento del cliente encaja de forma natural con el concepto 
de Skill de Claude Cowork: un flujo de trabajo reutilizable que 
recibe una transcripción como entrada, tiene acceso a 
herramientas de escritura de archivos, y produce un documento 
Word estructurado con el formato específico de Colb.AI. A esto 
se suma que Claude Cowork está incluido en la suscripción de la 
empresa, lo que elimina cualquier coste adicional. Al igual que 
en el módulo de toma de requerimientos, se prevé desarrollar 
también una implementación propia basada en Python que replique 
este flujo de forma independiente de cualquier suscripción 
comercial, siguiendo la misma arquitectura de pipeline descrita 
en la sección \ref{sec:toma_requerimientos_cap}. 

\subsection{Impacto en la planificación, costes y metodología}
\label{sec:impacto_planificacion}

\subsubsection{Impacto en los objetivos.} El cambio de alcance no 
invalida los objetivos del GEP sino que los reorienta. Los 
sub-objetivos de coherencia arquitectónica, trazabilidad entre 
especificación y código, y modularidad del sistema se mantienen 
intactos y se materializan ahora mediante la configuración de 
herramientas existentes en lugar del desarrollo de un asistente 
propio. El alcance se amplía además con dos pilares nuevos 
---toma de requerimientos y seguimiento del cliente--- que no 
estaban contemplados en la planificación inicial.

\subsubsection{Impacto en los costes.} El presupuesto estimado de 
14.099,81\verb|€| definido en el GEP se mantiene como referencia 
válida. El cambio de alcance tiene un impacto económico 
prácticamente neutro: las horas previstas para el desarrollo 
de agentes desde cero se redistribuyen entre la investigación 
del ecosistema de extensibilidad, la integración de herramientas 
externas y la implementación de los módulos de toma de 
requerimientos y seguimiento del cliente. La estructura de 
costes de personal, costes genéricos y contingencias no varía 
de forma significativa.

\subsubsection{Metodología.} La metodología SCRUM adoptada en el GEP 
no ha experimentado ningún cambio. El propio mecanismo de 
revisión de sprint fue el que permitió absorber el cambio de 
alcance de forma controlada: al finalizar la fase TP1, los 
hallazgos sobre la extensibilidad de los asistentes CLI se 
presentaron al director del proyecto y a Colb.AI, y el Sprint 
Backlog se reorientó en consecuencia.

\subsubsection{Estado actual del proyecto.} El módulo de transcripción 
está implementado y pendiente de revisiones finales. colbPowers, 
con el flujo SDD completo de skills y servidores MCP, está 
implementado y actualmente en fase de pruebas y evaluación, 
sujeto a ajustes en caso de detectarse fallos o mejoras. Para 
la evaluación de estas pruebas se utilizará SCC 
\cite{boyter2025scc}, una herramienta open source de análisis 
estático que calcula métricas de calidad del código como 
líneas de código, complejidad y cobertura, ya integrada en el 
sistema. El módulo de seguimiento del cliente está pendiente 
de implementación; se prevé que sea relativamente sencillo 
dado que su estructura es análoga a la del módulo de toma de 
requerimientos: recibe una transcripción o resumen como entrada 
y produce un documento de salida con un formato predefinido. 
El estado detallado de cada módulo se recoge en el 
capítulo \ref{sec:estado_implementacion}.
...
% TODO: cite filtered ClaudeCode repo?
...
El experimento sigue un diseño de cuatro condiciones (A/B/C/D) con cuatro repeticiones por condición (n=3), ejecutadas con temperatura 0 para minimizar la varianza estocástica del modelo. Las cuatro condiciones son:

\begin{table}[H]
\centering
\begin{tabular}{|p{1.2cm}|p{4.8cm}|p{8.2cm}|}
\hline
\textbf{Cond.} & \textbf{Herramientas activas} & \textbf{Descripción} \\
\hline
A & Herramientas nativas CLI & Solo Read, Write, Edit, Bash, Grep. Sin plugins ni servidores MCP adicionales. Actúa como baseline. \\
\hline
B & Nativas + code-review-graph & Añade el servidor MCP de code-review-graph. El agente puede consultar el grafo de dependencias, calcular el blast radius de los cambios y realizar búsquedas semánticas sobre el código. \\
\hline
C & Nativas + colbPowers & El agente opera bajo el flujo SDD de colbPowers, sin acceso a las herramientas de exploración de código de code-review-graph. \\
\hline
D & Nativas + colbPowers + code-review-graph & Combinación completa. El agente sigue el flujo SDD de colbPowers con acceso además al servidor MCP de code-review-graph. La tarea sigue el ciclo: constitution $\rightarrow$ features $\rightarrow$ spec $\rightarrow$ plan $\rightarrow$ execute $\rightarrow$ review. \\
\hline
\end{tabular}
\caption{Condiciones experimentales del diseño A/B/C/D}
...
\subsubsection{Caso 1: Aplicación Flutter}
\label{sec:caso1_confortmental}
El primer experimento se llevará a cabo sobre un repositorio real de Colb.Ai. Con tal de respetar las políticas de confidencialidad, la descripción del proyecto será lo suficientemente detallada como para entender el contexto, pero se omitirán ciertos detalles con tal de anonimizarlo. 

El proyecto consiste en una aplicación de escritorio desarrollada con el SDK \textit{Flutter}. El experimento consistirá en pedirle al asistente de IA que haga una lista de cambios considerablemente extensa a lo largo de las distintas páginas de la aplicación; principalmente cambios visuales. La hipótesis inicial es que un agente de código básico, se olvida rápidamente de muchos de los cambios pedidos (ya sea por alucinaciones o por ventana de contexto), en cambio agentes que siguen la metodología estructurada de colbPowers, documentando los cambios exactos que deben hacer antes de hacerlos, no sufrirán de este problema. Además, también se espera ver una reducción de consumo de tokens en las configuraciones que utilizan code-review-graph, puesto que en teoría esta herramienta es ideal para una tarea como esta, que requiere explorar distintos archivos del repositorio.
...
\hline
\textbf{Métrica} & \textbf{Cond. A} & \textbf{Cond. B} & \textbf{Cond. C} & \textbf{Cond. D} \\
 & \textbf{(sin plugins)} & \textbf{(code-review-graph)} & \textbf{(colbPowers)} & \textbf{(colbPowers + CRG)} \\
\hline
Tokens entrada (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tokens salida (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls totales (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls CRG (media) & N/A & {[}PENDIENTE{]} & N/A & {[}PENDIENTE{]} \\
Cobertura (media \%) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
\hline
\end{tabular}
}
\caption{Resultados Caso 1: Aplicación Flutter}
\label{tab:caso1_resultados}
\end{table}

\subsubsection{Caso 2: Aplicación de simulación 3D}
\label{sec:caso2_simulacion3d}
El segundo caso de uso aplica el experimento sobre una tarea de nueva implementación desde cero: el desarrollo de una aplicación de simulación 3D en la que, al pulsar una tecla, aparece una esfera en el entorno tridimensional; al hacer clic sobre ella, la esfera explota en una animación de partículas. A diferencia del caso anterior (modificación de una base de código existente), este caso evalúa el comportamiento de las herramientas en la creación de funcionalidad nueva.

Este caso es especialmente relevante para evaluar colbPowers, ya que las condiciones C y D 
pueden ejecutar el ciclo completo de la metodología SDD: primero se generan de forma 
interactiva con el usuario los documentos de constitución y \textit{features} del 
proyecto, y a continuación se genera la especificación técnica de la funcionalidad 
a implementar. En todo momento el proceso sigue el principio de \textit{Human-in-the-Loop}: 
el usuario participa en cada etapa y puede controlar el flujo. La hipótesis es que 
las condiciones C y D producen código de mayor calidad estructural (menor complejidad 
ciclomática, mejor índice de mantenibilidad) y mayor trazabilidad entre las 
decisiones de diseño y el código implementado.
...
Tokens salida (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls totales (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Cobertura funcional (media \%) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Complejidad ciclomática (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Índice de mantenibilidad (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
\hline
\end{tabular}%
}
...
\begin{itemize}
    \item \textbf{Compresión de datos e imagenes (CDI):} Para la implementación propia de el módulo de transcripción, se utilizan ffmpeg con algunas tecnicas de compresión vistas en CDI, para que el proceso de transcripción tarde menos sin perder demasiada calidad en la transcripción. Principalmente, se hace un \textit{Donwsampling} -Reducción en el muestreo del audio-, luego se convierte a MP3 para que utilice \textit{Huffman Encoding} para reducir aún mas el tamaño. 
...
\textbf{Métricas de calidad de código:} La integración de las métricas ya está cubierta. Gracias al repo opensource de SCC.

\textbf{Estado:} Funcional. En proceso iterativo activo.
