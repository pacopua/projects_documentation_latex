...
Con el objetivo de evaluar empíricamente el impacto de las herramientas desarrolladas, se ha ejecutado un experimento controlado que compara dos condiciones de implementación ---línea base sin plugins frente a la combinación colbPowers~+~code-review-graph--- sobre dos casos de uso distintos, utilizando dos asistentes de programación diferentes.

\subsection{Diseño experimental}
\label{sec:diseno_experimental}
El experimento compara dos condiciones (A y D) con tres repeticiones por condición (n=3), ejecutadas con temperatura~0 para minimizar la varianza estocástica del modelo. Las condiciones son:
...
\hline
A & Herramientas nativas CLI & Solo Read, Write, Edit, Bash, Grep. Sin plugins ni servidores MCP adicionales. Actúa como línea base. \\
\hline
D & Nativas + colbPowers + code-review-graph & Combinación completa. El agente sigue el flujo SDD de colbPowers con acceso al servidor MCP de code-review-graph. La tarea sigue el ciclo: constitution $\rightarrow$ features $\rightarrow$ spec $\rightarrow$ plan $\rightarrow$ execute $\rightarrow$ review. \\
\hline
\end{tabular}
\caption{Condiciones experimentales del diseño A/D}
\label{tab:condiciones_experimentales}
\end{table}

Cada condición se ha ejecutado con dos asistentes distintos: \textbf{OpenCode con DeepSeek V4-Flash} y \textbf{ClaudeCode con Haiku~4.5}. Esto permite observar el impacto de los plugins de forma independiente al modelo de lenguaje subyacente.

Las métricas recogidas en cada ejecución son: tokens totales consumidos (incluyendo tokens de caché), número de tool calls del orquestador, y número de llamadas a herramientas de code-review-graph.
...
\hline
\textbf{Métrica} & \textbf{OpenCode} & \textbf{OpenCode} & \textbf{ClaudeCode} & \textbf{ClaudeCode} \\
 & \textbf{Cond.~A} & \textbf{Cond.~D} & \textbf{Cond.~A} & \textbf{Cond.~D} \\
\hline
Tokens totales (media, M) & 3,26 & 9,38 & 3,74 & 14,33 \\
Tool calls orch. (media) & 69 & 92 & 53 & 63 \\
Llamadas CRG (media) & 0 & 0 & 0 & 1,0 \\
\hline
\end{tabular}
}
\caption{Resultados Caso 1: Aplicación Flutter (n=3 por condición y asistente)}
\label{tab:caso1_resultados}
\end{table}

En la condición D se observa un incremento muy notable en el consumo de tokens respecto a la condición A: ×2,9 para OpenCode y ×3,8 para ClaudeCode. Este sobrecoste refleja el overhead del flujo SDD de colbPowers, que genera documentación estructurada y delega la implementación a múltiples subagentes. El número de tool calls del orquestador crece de forma más moderada, ya que en la condición D parte del trabajo se distribuye entre subagentes. Destaca que las llamadas a code-review-graph fueron prácticamente nulas (0 en OpenCode, 1,0 de media en ClaudeCode), lo que sugiere que los agentes no incorporaron de forma proactiva el grafo de dependencias para una tarea de tipo exploratorio-visual.

\subsection{Caso 2: Aplicación de simulación 3D}
\label{sec:caso2_simulacion3d}
El segundo caso aplica el experimento sobre una tarea de nueva implementación desde cero: el desarrollo de una aplicación de simulación 3D en la que, al pulsar una tecla, aparece una esfera en el entorno tridimensional; al hacer clic sobre ella, la esfera explota en una animación de partículas. A diferencia del caso anterior (modificación de una base de código existente), este caso evalúa el comportamiento de las herramientas en la creación de funcionalidad nueva.

Este caso es especialmente relevante para evaluar colbPowers, ya que la condición D
puede ejecutar el ciclo completo de la metodología SDD: primero se generan de forma
...
el usuario participa en cada etapa y puede controlar el flujo.
...
\textbf{Métrica} & \textbf{OpenCode} & \textbf{OpenCode} & \textbf{ClaudeCode} & \textbf{ClaudeCode} \\
 & \textbf{Cond.~A} & \textbf{Cond.~D} & \textbf{Cond.~A} & \textbf{Cond.~D} \\
\hline
Tokens totales (media, M) & 1,88 & 8,41 & 3,74 & 8,19 \\
Tool calls orch. (media) & 52 & 85 & 63 & 48 \\
Llamadas CRG (media) & 0 & 0,7 & 0 & 0,3 \\
\hline
\end{tabular}%
}
\caption{Resultados Caso 2: Aplicación de simulación 3D (n=3 por condición y asistente)}
\label{tab:caso2_resultados}
\end{table}

En el segundo caso el incremento de tokens en la condición D es también significativo (×4,5 para OpenCode y ×2,2 para ClaudeCode). El número de tool calls del orquestador en la condición D es inferior al de la condición A para ClaudeCode (48 frente a 63), lo que refleja que el orquestador de colbPowers delega la mayor parte de las operaciones a subagentes especializados, reduciendo su propia carga de acciones directas. Las llamadas a code-review-graph son igualmente bajas (0,7 y 0,3 de media para OpenCode y ClaudeCode respectivamente), lo que es coherente dado que se trata de un proyecto nuevo sin base de código preexistente que explorar.
