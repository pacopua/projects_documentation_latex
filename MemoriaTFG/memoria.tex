...
La integración de esta herramienta aborda directamente el problema de gestión de la ventana de contexto identificado en el apartado de riesgos (sección~\ref{sec:riesgo5}). En pruebas preliminares realizadas sobre un monorepo de \textit{Colb.AI}, el agente con code-review-graph integrado necesitó aproximadamente 40.000 tokens para describir la arquitectura del proyecto, frente a los 64.000 tokens requeridos sin la herramienta, lo que supone una reducción del 37,5\verb|%| en esa prueba concreta. Las capacidades que expone al agente incluyen:
\begin{itemize}
...
\textit{Análisis estadístico.} En baseline las diferencias entre asistentes presentan efecto grande, con $p$-values indicativos de $p=0{,}007$ (complejidad) y $p=0{,}039$ (mantenibilidad). Con colbPowers, la $d$ de Cohen cae a valores próximos a cero en ambas métricas.

% ── Caso 1 ──────────────────────────────────────────────────────────────────

\paragraph{Caso~1 (Repo Existente) --- Baseline vs.\ colbPowers.}
En el Caso~1 el patrón se invierte respecto al Caso~2: colbPowers produce mayor complejidad ciclomática que baseline tanto en OpenCode (3,37 vs.\ 2,67, $+$26\%) como en ClaudeCode (3,48 vs.\ 2,98, $+$17\%), mientras que el índice de mantenibilidad se mantiene prácticamente igual (OpenCode: 66,59 vs.\ 67,59; ClaudeCode: 66,52 vs.\ 66,05). Este resultado no confirma la hipótesis de que \textit{code-review-graph} proporcionaría un contexto de dependencias superior: las llamadas al servidor MCP fueron prácticamente nulas en todas las ejecuciones, lo que privó al flujo SDD de la ventaja de exploración que habría justificado su overhead.
...
Las Figuras~\ref{fig:ttest_cpbl_oc_cm} y~\ref{fig:ttest_cpbl_cc_cm} (presentadas en la sección de análisis de consumo) muestran los paneles completos con barras de error (SD) y la $d$ de Cohen para cada comparación.

\paragraph{Caso~1 --- OpenCode vs.\ ClaudeCode.}
En el Caso~1 las diferencias de calidad entre asistentes son menos pronunciadas que en el Caso~2. En baseline, ambos producen complejidad ciclomática similar (2,67 OpenCode, 2,98 ClaudeCode) e índice de mantenibilidad comparable (67,59 vs.\ 66,05). Con colbPowers la brecha se mantiene pequeña (3,37 vs.\ 3,48 en complejidad; 66,59 vs.\ 66,52 en mantenibilidad), lo que sugiere que en tareas de modificación incremental la elección del asistente tiene menor impacto en la calidad estructural del código producido.
...
La evaluación experimental produce resultados mixtos. Dado el tamaño muestral ($n=3$ por condición), la $d$ de Cohen se utiliza como métrica principal del efecto, mientras que los $p$-values test $t$ de Welch se reportan como referencia indicativa. El hallazgo principal es la mejora con tamaño de efecto grande en las métricas de calidad del código en el Caso~2 (proyecto generado desde cero): colbPowers reduce la complejidad ciclomática un 27\% y mejora el índice de mantenibilidad un 15\% en OpenCode, con $p$-values indicativos de $p=0{,}014$ y $p=0{,}037$ respectivamente. Sin embargo, con ClaudeCode, que de base produce código de mayor calidad, los efectos son fueron mucho menos noables, teniendo una subida en la compljida ciclomaica de un 9\% respecto al baseline, y una mejora de el indice de mantenibilidad de un 3\%, sin embargo, siendo esta una diferencia tan reducida, con un soporte estadístico de $n=3$, no se pueden extraer resultados definitivos. En el Caso~1 (eliminación de funcionalidades complejas sobre un repositorio existente), el patrón se invierte: colbPowers produce código de mayor complejidad ciclomática (+17--26\%) sin mejora neta en mantenibilidad. El factor determinante parece ser que los agentes no invocaron \textit{code-review-graph} de forma efectiva, privando al flujo SDD del contexto de dependencias que habría justificado su overhead. Sin ese contexto, la planificación estructurada generó código de limpieza adicional más complejo que las eliminaciones directas del baseline.
...
\begin{figure}[H]
    ...
    \caption{Análisis estadístico: colbPowers vs.\ baseline. Se incluye la $d$ de Cohen como métrica principal del tamaño de efecto.}
...
\begin{figure}[H]
    ...
    \caption{Análisis estadístico: OpenCode vs.\ ClaudeCode. Se incluye la $d$ de Cohen como métrica principal del tamaño de efecto.}
