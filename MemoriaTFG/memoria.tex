...
Las métricas de calidad de código del Caso~1, calculadas con DCM sobre el código modificado, muestran un patrón diferente al del Caso~2. En OpenCode, colbPowers produce código de \emph{mayor} complejidad ciclomática (3,37 frente a 2,67 del baseline, +26\%) y un índice de mantenibilidad ligeramente inferior (66,59 frente a 67,59, $-$1\%). En ClaudeCode el incremento de complejidad es similar (3,48 frente a 2,98, +17\%), mientras que el índice de mantenibilidad se mantiene prácticamente igual (66,52 frente a 66,05). Este comportamiento contrasta con el Caso~2: en tareas de modificación incremental sobre código existente, la estructuración adicional de colbPowers puede introducir abstracciones o ramas de control adicionales que incrementan la complejidad medida; en cambio, cuando el agente diseña la arquitectura completa desde cero (Caso~2), el efecto predominante es la reducción de soluciones ad-hoc y de funciones innecesariamente complejas. La Figura~\ref{fig:ttest_cpbl_oc_cm} y la Figura~\ref{fig:ttest_cpbl_cc_cm} muestran los cuatro paneles de métricas para el Caso~1.
...
En el Caso~1 el efecto de colbPowers sobre las métricas de calidad es opuesto al del Caso~2: la complejidad ciclomática es mayor en colbPowers que en baseline, tanto en OpenCode (3,37 vs.\ 2,67, $+$26\%) como en ClaudeCode (3,48 vs.\ 2,98, $+$17\%). El índice de mantenibilidad se mantiene prácticamente sin cambios en ClaudeCode (66,52 vs.\ 66,05) y cae marginalmente en OpenCode (66,59 vs.\ 67,59, $-$1\%). Los paneles de calidad en las Figuras~\ref{fig:ttest_cpbl_oc_cm} y~\ref{fig:ttest_cpbl_cc_cm} muestran estos valores con barras de error (SD) y la $d$ de Cohen correspondiente.
