...
{\textbf{Abstract}}
\end{center}
The present project consists of the design and application of an intelligent assistance system, with the objective of automating the various stages in the development of software projects ---requirements gathering, Spec-Driven Development, and client follow-up--- through the orchestration of generative Artificial Intelligence tools. Placing emphasis on the interaction with the human developer, following a philosophy of \textit{Human in the loop}, and the generation of documentation to guide the process. This methodology aims to reduce the quality loss typically caused by the use of AI in the development of these projects. The experimental evaluation conducted suggests improvements in code quality and maintainability in greenfield scenarios, with mixed results in refactoring tasks when using the proposed system. This final degree project is applied and developed for the working environment at the software consulting company \textit{Colb.AI}.

\medskip
{
\small
\noindent\textbf{Keywords:} Spec-Driven Development (SDD), Large Language Models (LLM), Coding agents, Prompt engineering, Human-in-the-Loop (HITL), Software development
}
\vfill
\hspace{0pt}
}
...
\textbf{CC}     & Complejidad Ciclomática \\
\textbf{CLI}    & Command-Line Interface \\
\textbf{CRG}    & Code-Review-Graph \\
\textbf{DCM}    & Dart Code Metrics \\
\textbf{HITL}   & Human-In-The-Loop \\
\textbf{IDE}    & Integrated Development Environment \\
\textbf{LLM}    & Large Language Model \\
\textbf{LoC}    & Lines of Code \\
\textbf{MCP}    & Model Context Protocol \\
\textbf{MI}     & Maintainability Index \\
\textbf{MVP}    & Minimum Viable Product \\
\textbf{ReAct}  & Reasoning + Acting \\
\textbf{SAS}    & Shared Access Signature \\
\textbf{SDK}    & Software Development Kit \\
\textbf{SDD}    & Spec-Driven Development \\
...
Este trabajo de final de grado consiste en la creación de un sistema de automatización para el desarrollo y diseño de proyectos de software, utilizando como pilar principal herramientas basadas en \textit{LLMs}. El proyecto se enmarca para su uso dentro de una empresa real, \textit{Colb.AI}, dedicada principalmente a la consultoría de proyectos de software.

Es innegable que las herramientas de inteligencia artificial, principalmente los \textit{Large Language Models}, están cambiando la forma en que se desarrollan los proyectos de software para la mayor parte de las empresas del sector \cite{jetbrains_deveco_2025}. Estas tecnologías presentan la oportunidad de automatizar muchas de las tareas que tradicionalmente habían sido llevadas a cabo exclusivamente por los desarrolladores humanos, pero su adopción sin la supervisión humana adecuada conlleva riesgos que pueden comprometer la calidad y sostenibilidad del software generado, sobre todo a largo plazo.

Se presentan a continuación tres módulos que se han automatizado en el desarrollo de este TFG:
\begin{enumerate}
    \item \textbf{Toma de requerimientos y diseño de arquitectura:} Necesidades del cliente a partir de la transcripción de reuniones, exploración de \textit{frameworks} posibles a utilizar, y diseño de la arquitectura del proyecto.
...
Aunque existen herramientas que sí exigen especificación previa como \textit{Spec Kit} o \textit{AgentOS}, estas no se adaptaban adecuadamente al flujo de trabajo de \textit{Colb.AI}, su modelo de documentación era demasiado rígido y generaba una fricción excesiva incluso para cambios menores.
...
El \textit{Spec-Driven Development} (SDD) es una filosofía donde la documentación detallada de los requisitos y el comportamiento esperado del sistema (la especificación) dirige la creación del software. A diferencia del enfoque tradicional, donde estos documentos descriptivos solían quedar subordinados o desactualizarse frente al código, el SDD invierte esta relación. El documento estructurado en lenguaje natural actúa como la base de la verdad sobre la que se construye el código. \cite{ostroff_agile_2004} \cite{github_spec_kit}

En otras palabras, hay que tener definido previamente exactamente qué queremos que haga cada funcionalidad de código, y cómo queremos que lo haga. Cuando se quiera hacer cambios en la implementación, se han de aplicar primero en los documentos de especificación y luego en el código del proyecto. \\
...
\begin{itemize}
    ...
    \item \textbf{Agentes de Código:} Un agente es un asistente al que se le asigna un rol mediante un \textit{prompt} y se le equipa con distintas \textit{Agent Skills} y herramientas varias. Uno de sus usos mas útiles es que un agente orquestador lo pueda invocar para que tenga un contexto limpio, y así no esté sesgado por la conversación que se ha tenido previamente. Su propósito es llevar a cabo una tarea específica. Además, es posible encadenar varios agentes especializados para construir un sistema coordinado. \cite{vscode_agents} \cite{wang_llm_agents_2024}

    \item \textbf{Habilidades del Agente (\textit{Agent Skills}):} Son documentos de instrucciones procedimentales en formato \textit{Markdown} que el agente consulta bajo demanda cuando necesita ejecutar un tipo de tarea específico. Al activar una \textit{Skill}, el agente lee primero su contenido, que define \textit{cómo} debe razonar y actuar ante esa clase de tarea, y puede además invocar \textit{scripts} o plantillas incluidos en la propia \textit{Skill} durante su ejecución. \cite{agentskills_home} \cite{yao_react_2023}
\end{itemize}
...
\subsubsection{Asistentes Comerciales}
...
Para reuniones telemáticas, las plataformas de videoconferencia más extendidas (Microsoft Teams, Google Meet, Zoom) ofrecen transcripción automática integrada con diarización nativa, ya que cada participante habla desde su propio dispositivo. Para reuniones presenciales, las alternativas principales son Whisper \cite{radford2022whisper}, el modelo de reconocimiento de voz de OpenAI (ejecución local, sin coste de API, pero sin diarización nativa), y \textbf{Azure Speech Service} (servicio cloud con diarización de hablantes). Se optó por Azure Speech Service al estar incluido en la suscripción de Azure de \textit{Colb.AI} y por ofrecer resultados satisfactorios en las pruebas realizadas.

\subsubsection{Herramientas de documentación de reuniones}
\label{sec:herramientas_documentacion_reuniones}
Existen herramientas comerciales orientadas a la generación automática de resúmenes y documentación post-reunión, como Otter.ai \cite{otter_ai_2026}, Fireflies.ai \cite{fireflies_ai_2026} o Microsoft Copilot for Teams. Estas soluciones se descartaron por dos razones complementarias: en primer lugar, generan resúmenes genéricos, y en segundo lugar, ya se disponía de Claude Cowork con skills, capaz de generar documentación de seguimiento con la misma eficacia a través de un skill dedicado. Por otro lado, la implementación de un módulo Python propio para este fin no supone una complejidad técnica elevada, lo que refuerza la decisión de desarrollar una solución a medida en lugar de depender de una herramienta externa.

\subsubsection{Claude Cowork}
\label{sec:claude_cowork}
Claude Cowork \cite{anthropic_claude_cowork} es una herramienta de Anthropic orientada a la automatización de tareas en entorno de escritorio. Su modelo de uso se basa en skills que pueden invocar herramientas nativas de acceso al sistema de archivos, búsqueda en internet y ejecución de scripts, lo que la convierte en un entorno idóneo para implementar flujos de trabajo ligados al ciclo de vida del desarrollo de software.

\subsubsection{Asistentes CLI extensibles}
\label{sec:asistentes_cli}
Las herramientas de asistencia a la programación de tipo CLI (\textit{Command-Line Interface}) representan una categoría diferenciada de los asistentes integrados en el IDE. Operan directamente desde la terminal con acceso nativo al sistema de archivos y exponen mecanismos de extensibilidad nativos ---plugins, hooks de ciclo de vida y el protocolo MCP (\textit{Model Context Protocol} \cite{anthropic_mcp_2024})--- que permiten definir flujos de trabajo de programación estructurados de forma composable. Las dos principales herramientas de este tipo son ClaudeCode \cite{claude_code_2024} (Anthropic) y OpenCode \cite{opencode_2025} (open source, compatible con modelos locales y remotos).

\subsubsection{superpowers plugin}
\label{sec:superpowers}
\textit{superpowers} \cite{superpowers_2025} es un plugin open source para ClaudeCode que implementa el patrón orquestador + subagentes mediante skills. Proporciona una arquitectura base para flujos de trabajo \textit{Spec-First}, en la que un agente orquestador coordina subagentes especializados a través de skills reutilizables. Sin embargo, carece de un mecanismo de constitución ni de features compartidas entre sesiones o desarrolladores distintos, lo que limita su capacidad para garantizar coherencia arquitectónica a lo largo del ciclo de vida de un proyecto.

\subsubsection{Enfoques académicos: planificación previa y agentes múltiples}
\label{sec:enfoques_academicos}

Paralelamente al desarrollo de herramientas comerciales, la comunidad académica ha explorado enfoques estructurados para mejorar la calidad del código generado por LLMs, aportando evidencia empírica sobre las limitaciones del paradigma \textit{Code-Driven} y sobre la viabilidad de enfoques más estructurados.

\paragraph{Planificación previa a la generación de código.}
Jiang et al.\ \cite{jiang2023selfplanning} proponen un enfoque de dos fases ---planificar y luego generar--- en el que el LLM produce primero un plan estructurado y después genera el código guiado por él. Los resultados muestran mejoras de hasta un 25,4\% frente a la generación directa y un 11,9\% frente a \textit{chain-of-thought}, con evaluadores humanos confirmando ganancias en corrección, legibilidad y robustez del código producido. Este trabajo ofrece validación empírica directa al principio central del SDD: imponer una fase de especificación previa a la implementación mejora sistemáticamente la calidad del resultado.

\paragraph{Sistemas multi-agente para el ciclo de vida del software.}
MetaGPT \cite{hong2023metagpt} codifica procedimientos operativos estándar (SOPs) en secuencias de \textit{prompts} y asigna roles diferenciados a agentes LLM organizados en \textit{pipeline}, reduciendo las alucinaciones en cascada que surgen al encadenar modelos de lenguaje de forma directa. ChatDev \cite{qian2024chatdev} orquesta agentes especializados mediante una cadena de comunicación que guía el desarrollo a través de fases (diseño, codificación y pruebas), demostrando que la comunicación estructurada entre agentes es eficaz a lo largo de todo el ciclo de vida del software. Ambos trabajos comparten con \textit{colbPowers} la idea de que la separación de responsabilidades entre agentes especializados y la imposición de un protocolo estructurado producen código de mayor calidad que la generación directa.

\paragraph{Evaluación de agentes en escenarios reales.}
Jimenez et al.\ \cite{jimenez2024swebench} introdujeron SWE-bench, un \textit{benchmark} de 2.294 problemas reales extraídos de repositorios GitHub, y observaron que el modelo más capaz en el momento de la publicación solo resolvía el 1,96\% de los casos de forma autónoma. Este resultado ilustra la brecha entre la generación de código en aislamiento y la resolución de tareas de ingeniería reales, y contextualiza por qué la supervisión humana estructurada ---\textit{Human in the Loop}--- sigue siendo indispensable en los flujos de desarrollo asistidos por IA.

\subsection{Análisis comparativo: ¿Está justificada una solución a medida?}
\label{sec:analisis_comparativo}

Si bien las alternativas mencionadas presentan avances significativos, su aplicación directa en nuestro entorno de trabajo plantea diversos desafíos operativos y de arquitectura. 

En primer lugar, los asistentes puramente comerciales fomentan un ciclo de desarrollo directo e impulsivo. Al no exigir una especificación estructurada previa, agravan el problema del \textit{slop code} (código descuidado o de baja calidad estructural) mencionado en la introducción. Por otro lado, herramientas enfocadas en la especificación como Spec Kit generan una fricción operativa excesiva, produciendo una cantidad desproporcionada de documentos incluso para implementar \textit{features} muy sencillas, lo que ralentiza el desarrollo ágil. Finalmente, marcos de trabajo como AgentOS están diseñados bajo un enfoque \textit{greenfield} (proyectos creados desde cero), resultando sumamente complejos de integrar en repositorios ya existentes.

En la Tabla \ref{tab:comparativa_herramientas} se presenta un resumen de las características y limitaciones que estas soluciones presentan:
...
De las herramientas analizadas, \textit{superpowers} fue evaluado como candidato directo al problema planteado. Resuelve la orquestación de agentes, pero no garantiza coherencia arquitectónica entre sesiones ni entre desarrolladores distintos, ya que carece de un mecanismo de contexto persistente y compartido. Se decidió adoptarlo como base y extenderlo: el resultado es \textbf{colbPowers}, que añade la capa de \texttt{constitution.md} (reglas y estándares del proyecto) y \texttt{features.md}, y ha requerido tanto la modificación de skills ya existentes como la creación de nuevas skills ---\textit{defining-constitution} y \textit{defining-features}--- para que estos documentos se integren adecuadamente en el flujo del agente y sean consultados de forma sistemática en cada fase del ciclo SDD.

A la vista de estas limitaciones, la creación de una solución propia resulta razonable. Desarrollar nuestro propio sistema nos proporciona la flexibilidad indispensable para integrarlo de forma nativa con nuestro \textit{pipeline} de trabajo.
...
Aprovechando la arquitectura de \textit{superpowers}, el proyecto propone el desarrollo de \textbf{colbPowers}: un plugin para ClaudeCode y OpenCode que extiende el patrón orquestador/subagentes añadiendo una capa de contexto persistente basada en \texttt{constitution.md} y \texttt{features.md}, y creando y modificando las skills necesarias para que el agente incorpore estos documentos de forma sistemática en su flujo de trabajo. El ciclo de vida del software girará en torno a tres pilares complementarios:
...
\begin{enumerate}
    ...
    \item \textbf{Spec-Driven Development:} usar \textit{colbPowers} para garantizar que cada implementación esté precedida de una especificación validada, coordinada por un agente orquestador que invoca subagentes especializados (implementador, revisor de código, revisor de compatibilidad con specs/constitución/features). 
    \item \textbf{Seguimiento del cliente:} automatizar la generación de documentación post-reunión con el formato específico de \textit{Colb.AI}.
\end{enumerate}

Originalmente se contempló construir este sistema desde cero. Durante la fase de investigación previa (TP1) se descubrió que \textit{superpowers} ya ofrecía la arquitectura base necesaria, lo que permitió redirigir el esfuerzo hacia la contribución diferencial: la capa de constitución y features ---junto con las skills que hubo que crear y modificar para incorporar estos documentos al flujo del agente--- y los tres pilares del ciclo de vida del software.

Cabe destacar que este trabajo gira principalmente en torno al \textbf{aprovechamiento} de herramientas ya existentes. El esfuerzo se ha centrado en la integración de dichas herramientas en un flujo de trabajo cohesivo, en la modificación de aquellas que lo requerían para adaptarse al flujo propuesto, y en la creación de la infraestructura necesaria para que operen como un sistema completo.

\subsection{Flujo de trabajo completo}
\label{sec:flujo_completo}

La Figura~\ref{fig:flujo_completo} muestra el flujo de trabajo del sistema de principio a fin. El proceso arranca con la \textbf{toma de requerimientos}: la primera reunión con el cliente se transcribe de forma diferente según su modalidad ---en reuniones telemáticas se aprovechan las transcripciones automáticas de plataformas como Teams o Meet, mientras que en reuniones presenciales se graba el audio y se procesa con Azure Speech Service \cite{azure_speech_2024} con diarización de hablantes--- y el texto resultante se envía el módulo de procesamiento, que puede ejecutarse mediante la \textit{skill} de Claude Cowork o mediante una implementación propia en Python con LangChain y Tavily \cite{tavily_2025}. En ambos casos, la salida es un plan de implementación estructurado que el usuario revisa y aprueba antes de continuar.

A partir de ese plan aprobado, el flujo entra en la fase de \textbf{desarrollo con colbPowers}: el agente orquestador guía al equipo a través del ciclo SDD completo ---definición del contexto del proyecto, especificación de la funcionalidad, implementación y revisión---, con puntos de validación humana en cada etapa que garantizan el principio de \textit{Human in the Loop} \cite{vaithilingam_2022}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{imagenes/DiagramaFlujoTrabajoCompleto.png}
...
En esta sección del trabajo se declaran los objetivos del proyecto, cuáles son los requerimientos para dar este objetivo por cumplido, las métricas que se utilizan para evaluar el sistema, y los posibles obstáculos y riesgos que pueden aparecer en el desarrollo del proyecto (Tal y como pide la metodología de \textit{GEP}). Adicionalmente, se ha añadido un último subapartado destacando la evolución que ha tenido este proyecto respecto la planificación inicial de \textit{GEP}.
...
\begin{enumerate}
    \item \textbf{Plugin colbPowers} para los asistentes CLI ClaudeCode y OpenCode: extiende \textit{superpowers} añadiendo \texttt{constitution.md} y \texttt{features.md} como contexto persistente compartido entre sesiones y desarrolladores, e implementa el flujo \textit{Spec-First} mediante un agente orquestador y subagentes especializados. La integración con OpenCode requirió el análisis en profundidad de su arquitectura interna ---disponible como software libre--- y el desarrollo manual de \textit{hooks} nativos en JavaScript. Este trabajo fue necesario para lograr la compatibilidad de OpenCode con herramientas como \textit{code-review-graph} \ref{sec:code_review_graph}, que implementan su integración mediante hooks nativos de ClaudeCode: dado que los sistemas de hooks de ambos asistentes difieren arquitectónicamente, fue necesario implementar los adaptadores equivalentes en JavaScript para que OpenCode pudiera invocar estos servidores MCP con el mismo comportamiento.
    \item \textbf{Módulo de toma de requerimientos}: transforma transcripciones de reuniones con el cliente en planes de implementación estructurados. Incluye un componente de transcripción con Azure Speech Service (con diarización de hablantes y gestión mediante gestión de estados de la transcripción y hashes SHA-256). Luego se utiliza una skill de Claude Cowork para diseñar un plan dee la toma de requerimientos, y alternativamente se implementa un pipeline propio —para evitar el \textit{vendor lock-in}— de procesamiento basado en LangChain y Tavily para búsqueda semántica y síntesis arquitectónica, con la misma finalidad que skill mencionada de Claude Cowork.
    \item \textbf{Módulo de seguimiento del cliente}: automatiza la generación de documentación post-reunión en el formato específico de \textit{Colb.AI}, disponible como skill de Claude Cowork y como implementación propia de código abierto con python-docx y langchain para el agente que procesa la transcripción.
\end{enumerate}

La validación de los resultados del sistema se realiza a través de ciertas métricas de análisis de calidad del código, conocidas y utilizadas en el paradigma del desarrollo de software. Además de, por supuesto, los revisores humanos, que han de analizar en todo momento el código generado y los resultados producidos, haciendo a este un sistema con lo que se conoce \textit{Human In The Loop} \cite{vaithilingam_2022}, cosa que significa que en todo momento hay supervisión e intervención humana. \\
...
\begin{itemize}
    ...
    \item El sistema debe incluir un módulo de generación de documentación de seguimiento post-reunión con el formato específico de \textit{Colb.AI}, a partir de la transcripción de reuniones periódicas con el cliente.
\end{itemize}
...
Para validar empíricamente la calidad del código generado por el sistema, se emplean métricas de análisis estático ampliamente adoptadas en la ingeniería del software \cite{sommerville_se_2015}. Estas métricas se calculan con herramientas de análisis estático según el lenguaje del proyecto: \textbf{radon} \cite{Lacchia_radon} para proyectos Python y \textbf{DCM --- Dart Code Metrics} \cite{dart_code_metrics} para proyectos Flutter/Dart. Es importante destacar que las métricas no forman parte del flujo automatizado de los agentes: constituyen una herramienta de evaluación independiente que el desarrollador utiliza para analizar manualmente la calidad del código generado y contrastar los resultados entre distintas ejecuciones del sistema.

Las métricas empleadas se definen formalmente a continuación.

\subsubsection{Complejidad Ciclomática}

La \textbf{Complejidad Ciclomática} \cite{McCabe_Complexity} de un módulo o función se define a partir de su grafo de flujo de control $G = (V, E)$, donde $V$ es el conjunto de nodos (bloques básicos de instrucciones) y $E$ es el conjunto de aristas (transferencias de control). La complejidad ciclomática $\mathcal{CC}(G)$ se calcula como:
...
\noindent donde $|E|$ es el número de aristas del grafo, $|V|$ es el número de nodos, y $p$ es el número de componentes conexas (típicamente $p = 1$ para una función individual). Esta métrica cuantifica el número de caminos de ejecución linealmente independientes dentro del módulo. Un valor elevado de $\mathcal{CC}(G)$ indica una lógica excesivamente ramificada, lo que dificulta las pruebas y el mantenimiento del código. Como referencia orientativa, se considera que valores $\mathcal{CC}(G) \leq 10$ corresponden a módulos simples y bien estructurados \cite{McCabe_Complexity}.

\subsubsection{Índice de Mantenibilidad}

El \textbf{Índice de Mantenibilidad} (\textit{Maintainability Index}) \cite{oman_maintainability_1992,microsoft_maintainability_index} es una métrica compuesta definida a partir de tres medidas base. Sea $\eta_1$ el número de operadores distintos, $\eta_2$ el número de operandos distintos, $N_1$ el número total de operadores y $N_2$ el número total de operandos de un módulo. El \textit{volumen de Halstead} \cite{halstead_1977} se define como:

\begin{equation}
    V_H = (N_1 + N_2)\log_2(\eta_1 + \eta_2)
    \label{eq:halstead_volume}
\end{equation}

Sean además $\mathcal{CC}$ la complejidad ciclomática del módulo (Ecuación~\ref{eq:cyclomatic}) y $\mathit{LoC}$ el número de líneas de código fuente. El Índice de Mantenibilidad se expresa como:

\begin{equation}
    \mathit{MI} = \max\!\left(0,\ \frac{171 - 5.2\ln(V_H) - 0.23\,\mathcal{CC} - 16.2\ln(\mathit{LoC})}{171} \times 100\right)
    \label{eq:mi}
\end{equation}

\noindent El resultado $\mathit{MI} \in [0, 100]$ proporciona un índice global de facilidad de mantenimiento del módulo analizado. Valores bajos (típicamente $\mathit{MI} < 20$ \cite{microsoft_maintainability_index}) indican código difícil de comprender y modificar.
...
\subsection{Integración de conocimientos}
\label{sec:integracion_conocimientos}
Como demanda la asignatura de \textit{GEP}, en esta sección se realiza un análisis de la integración de conocimientos adquiridos a lo largo del grado y su aplicación en el desarrollo de proyecto.
\subsubsection{Competencias técnicas del proyecto}
...
\begin{itemize}
    ...
    \item \textbf{Búsqueda y analisis de información masiva (CAIM):} Del apartado de \textit{Local Sensitive Hashing} de la asignatura, se extrajo la idea de almacenar los audios transcritos con sus respectivos hashes, aunque en la implementación no se utiliza ningún método de \textit{LSH}, se utiliza un hash criptográfico SHA-256 para identificación exacta del contenido. Los apartados de \textit{Web, Information Retrieval} fueron de utilidad para entender y implementar servicios como \textit{Tavily} para la búsqueda web, así como el temario de \textit{Networks} para la formación del grafo de \textit{code-review-graph}, que facilita a los asistentes de programación la búsqueda y exploración del código.
    \item \textbf{Introducción a la ingeniería del software (IES):} La base proporcionada en esta asignatura sobre requisitos de software, diseño de sistemas y creación de diagramas de flujo, fueron extremadamente útiles tanto para planificar el sistema implementado en este proyecto como para planificar el desarrollo del proyecto en si.
...
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
    ...
    \bottomrule
\end{tabularx}
\caption{Comparativa entre el alcance comprometido en el GEP y el alcance final ejecutado.}
\label{tab:comparativa_alcance}
\end{table}

La justificación del cambio es doble. Por un lado, la adopción de \textit{superpowers} como base eliminó la necesidad de construir la capa de orquestación desde cero, redirigiendo el esfuerzo hacia la contribución diferencial: la capa de \texttt{constitution.md} y \texttt{features.md}, así como la creación y modificación de skills para incorporar adecuadamente estos documentos al flujo del agente. Por otro lado, la reducción de esfuerzo en el núcleo SDD liberó capacidad para incorporar los módulos de toma de requerimientos y seguimiento del cliente, ampliando el alcance del proyecto para cubrir el ciclo de vida completo del software.
...
Para el desarrollo de este Trabajo de Final de Grado, se adopta una \textbf{metodología ágil}, específicamente en el marco de \textit{SCRUM}, orientada a la entrega continua de valor y a la adaptación rápida frente a posibles cambios. 
...
Una pieza fundamental de esta metodología son las reuniones diarias. Estas sesiones breves se llevan a cabo de forma telemática mediante \textbf{Microsoft Teams} junto al equipo de \textit{Colb.Ai}. En ellas se expone el progreso realizado el día anterior y se itera sobre las ideas del proceso actual. Esto asegura que el desarrollo se mantenga alineado con los objetivos de la empresa y los estándares de calidad esperados.
...
Para garantizar el correcto seguimiento del progreso del proyecto, se emplean las siguientes herramientas y dinámicas:

\begin{itemize}
    \item \textbf{Gestión de Tareas (Odoo):} La planificación ágil y el seguimiento de las tareas del proyecto se gestionan íntegramente a través de \textbf{Odoo}, la herramienta utilizada internamente por la empresa.
    
    \item \textbf{Control de Versiones (Git y GitHub):} El código fuente, la documentación y la evolución de los \textit{prompts} están controlados utilizando \textit{Git}. El repositorio remoto está alojado en los servidores de \textbf{GitHub}. Esta combinación facilita controlar y evaluar los avances que se hacen sobre el proyecto.
    
    \item \textbf{Reuniones de Seguimiento Académico:} Además de las interacciones diarias con la empresa, se establecen reuniones periódicas con el director académico del TFG. La frecuencia exacta de estos encuentros se va definiendo y ajustando de mutuo acuerdo a lo largo del desarrollo, con el objetivo de validar el rigor del proyecto, y revisar los entregables de documentación.
\end{itemize}
...
Como se ha mencionado al comienzo de este documento, la IA generativa está cambiando cómo trabajamos, la mayoría de estudiantes la utilizan ---aunque no todos de forma responsable---, para asistir en el desarrollo de sus proyectos. Este proyecto también la emplea, por lo que en aras a la transparencia, esta sección declara los usos que se le han dado a herramientas de inteligencia artificial generativa para el desarrollo de este TFG.
...
Se ha utilizado IA generativa para la asistencia en la redacción de esta memoria, principalmente para borradores iniciales, además de para reestructurar y reescribir algunas secciones. Todo el contenido generado se ha revisado, y el autor se hace responsable de la veracidad y exactitud de la información presentada. La IA se ha empleado como herramienta de apoyo, no como fuente primaria de contenido.

\paragraph{Exploración de ideas.}
Durante las fases de diseño se ha empleado IA generativa para explorar alternativas arquitectónicas, evaluar trade-offs entre opciones tecnológicas y evaluar el razonamiento tomar decisiones. Este proceso ha sido orientativo y exploratorio, y las decisiones finales se han tomado basándose en la experiencia del desarrollador y en la revisión de fuentes verídicas.
...
\begin{itemize}
    ...
    \item \textbf{TP2 - Planificación de estructura y flujo (20h)}: En esta etapa se planifica cuál es la estructura básica del proyecto y cómo es el flujo de utilización del producto final.
\end{itemize}
...
\begin{itemize}
    \item \textbf{DA1 - Sprint 1: MVP (50h)}: Adoptar \textit{superpowers} como base de colbPowers e instalar el plugin en ClaudeCode y OpenCode, definir la estructura de \texttt{constitution.md} y \texttt{features.md} como contexto persistente compartido entre sesiones y desarrolladores, y adaptar la skill \textit{using-superpowers} como \textit{using-colbPowers} para incorporar estos documentos al flujo del agente.
    \begin{itemize}
        ...
        \item \textbf{DA1.3} Adaptación de la skill \textit{using-superpowers} como \textit{using-colbPowers} e integración del hook \texttt{SessionStart} para la inyección automática de las reglas de orquestación al inicio de cada sesión (15h).
    \end{itemize}

    \item \textbf{DA2 - Sprint 2: Skills SDD y métricas de evaluación (50h)}: Crear las skills \textit{defining-constitution} y \textit{defining-features}, y adaptar las skills heredadas de \textit{superpowers} para que los subagentes incorporen \texttt{constitution.md}, \texttt{features.md} y los documentos de especificación en cada fase del ciclo SDD. Integrar las herramientas de análisis estático como mecanismo de evaluación independiente.
    \begin{itemize}
        \item \textbf{DA2.1} Creación de la skill \textit{defining-constitution} y adaptación de la skill de implementación de \textit{superpowers} para que el subagente incorpore \texttt{constitution.md} y los documentos de especificación como contexto (15h).
        \item \textbf{DA2.2} Creación de la skill \textit{defining-features} y adaptación de las skills revisoras de \textit{superpowers} para incorporar \texttt{features.md} y los documentos de especificación en la revisión de código y de coherencia con la constitución del proyecto (20h).
        \item \textbf{DA2.3} Desarrollo de scripts de análisis estático como herramienta de evaluación manual de la calidad del código generado: \textbf{radon} para Python y \textbf{DCM} (Dart Code Metrics) para Flutter/Dart (15h).
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{DA3.1} Desarrollo de \textit{skills} adicionales para colbPowers correspondientes a las fases del flujo SDD: \textit{brainstorming}, implementación y revisión de código contra especificaciones (30h).
        \item \textbf{DA3.2} Integración de \textit{code-review-graph} como servidor MCP y desarrollo de los adaptadores JavaScript necesarios para que OpenCode pudiera invocarlo con el mismo comportamiento que ClaudeCode (25h).
    ...
    \begin{itemize}
        \item \textbf{DA4.1} Módulo de transcripción: integración con Azure Speech Service con diarización, implementación de la gestión idempotente mediante máquina de estados en Azure Blob Storage y hashes SHA-256, y compresión determinista del audio con \texttt{ffmpeg} (35h).
        \item \textbf{DA4.2} Módulo de procesamiento de requerimientos: implementación del pipeline con LangChain, investigación y pruebas de integración de Tavily como motor de búsqueda semántica, y validación del flujo completo de generación del plan arquitectónico (25h).
...
\begin{itemize}
    ...
    \item \textbf{PV2 - Pruebas del bucle completo (Human in the Loop) (40h)}: Simulaciones de desarrollo de funcionalidades (\textit{features}) reales para auditar la colaboración entre el usuario y los agentes. Incluye la ejecución de los experimentos controlados (baseline vs.\ colbPowers), la recogida de métricas de calidad del código mediante análisis estático, y el análisis estadístico de los resultados (~20h).
\end{itemize}
...
\label{sec:recursos}
A continuación se listan los recursos Materiales, Humanos, y de software de los que depende el proyecto.
...
\begin{itemize}
    \item \textbf{Entorno de Desarrollo:} Sistema operativo Windows con Windows Subsystem for Linux (WSL) habilitado, proporcionando un entorno Unix nativo indispensable para la ejecución de \textit{scripts} en Bash y Python. El editor principal es Visual Studio Code (VSCode).
...
\textbf{DA2} & Sprint 2: Skills SDD y métricas & 50h & DA1 & Autor, PC, Python, radon \\
...
\bottomrule
\end{tabularx}
\caption{Resumen de las tareas del proyecto con sus dependencias y recursos asignados.}
...
\bottomrule
\end{tabularx}
\caption{Distribución estimada de horas del autor por rol a lo largo del proyecto.}
...
\begin{landscape}
    \begin{figure}[htbp] % Cambiamos [H] por [htbp]
     ...
     \caption{Diagrama de Gantt, creado con el software \textit{GanttProject}.}
     \label{fig:gantt}
    \end{figure}
\end{landscape}
...
\begin{tabularx}{\textwidth}{>{\centering\arraybackslash}p{1cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
    ...
    \midrule
    \textbf{DA1} & Agentes Architect, Developer y Reviewer desde cero & Adopción de \textit{superpowers} como base + capa \texttt{constitution.md}/\texttt{features.md} + inyección de \textit{using-colbPowers} mediante hook \texttt{SessionStart} \\
    \textbf{DA2} & Métricas de evaluación integradas en el agente Reviewer & Nuevas skills SDD (\textit{defining-constitution}, \textit{defining-features}) + adaptación de skills de \textit{superpowers} + scripts de análisis estático \\
    ...
    \bottomrule
\end{tabularx}
\caption{Comparativa entre la planificación del GEP y la planificación final ejecutada.}
...
Durante la planificación y ejecución de este proyecto, se identificaron diversos obstáculos potenciales inherentes al desarrollo con IA. A continuación, se detallan estos riesgos junto con sus respectivos planes alternativos, tal y como dictan las buenas prácticas de gestión ágil de proyectos. En el apartado \ref{sec:imprevistos} se cuantifica el coste económico estimado de cada uno de estos imprevistos.

Estos riesgos se definieron al desarrollar la planificación inicial del proyecto, y se indicará cuales de ellos han ocurrido y las consecuencias de esto.
...
En este apartado se presentan los distintos costes que aparecen a lo largo del desarrollo del proyecto.

\subsubsection{Coste del personal}
\label{sec:coste_personal}
Se divide este apartado en distintos roles. Cada uno tiene un coste por hora determinado, y a partir del diagrama de Gantt y las tablas con las horas asignadas a cada tarea y sus recursos personales adjuntados, se calcula el total de cada uno de estos roles. Adicionalmente, se muestran tanto los salarios netos como los brutos - añadida la contribución de 30\verb|%| a la Seguridad Social (SS) -, y cuando se hace la suma total de los costes, se utiliza obviamente el salario bruto.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{X c c}
         ...
         \bottomrule
    \end{tabularx}
    \caption{Costes de personal por hora según el rol asumido. Basada en información extraída de \cite{indeed_salarios}.}
    \label{tab:costes_personal}
\end{table}

\subsubsection{Coste por actividad}
\label{sec:coste_actividad}
Se repasan las actividades presentadas en el entregable anterior, los recursos humanos invertidos en cada una, y se calcula su coste.
...
\begin{itemize}
    ...
    \item \textbf{DA2 (Sprint 2 - Subagentes y métricas):} Consta de 50 horas. Las 15 horas iniciales de investigación y configuración de radon son llevadas a cabo por el \textit{Investigador y documentador}, mientras que las 35 horas restantes de implementación de subagentes y scripts recaen sobre el \textit{Programador junior}.
    \begin{equation*}
...
Para calcular el coste del hardware, no se imputa el precio total de compra de los equipos, ya que estos no agotan su vida útil exclusivamente con este proyecto. Se calcula su amortización. 
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{X c c c}
         ...
         \bottomrule
    \end{tabularx}
    \caption{Cálculo de la amortización de los recursos de \textit{Hardware}.}
...
La toma de requerimientos es la primera fase del ciclo de vida de un proyecto de software. En este contexto, el objetivo es transformar las necesidades expresadas por el cliente en documentos de especificación técnica utilizables por el equipo de desarrollo. El proceso se articula en dos módulos independientes y secuenciales: en primer lugar, el módulo de transcripción convierte las reuniones en texto estructurado; y en segundo lugar, el módulo de procesamiento analiza ese texto y genera el plan de implementación. Cada módulo dispone de dos implementaciones complementarias, con el propósito de no depender de un único proveedor, cubriendo así el riesgo de vendor lock-in mencionado anteriormente \ref{sec:riesgo2}.
...
\begin{enumerate}
    ...
    \item \textbf{Descarga y formateo del resultado}: una vez completado el trabajo, se descarga el fichero de resultados y se procesan las frases reconocidas (\texttt{recognizedPhrases}). Cuando la diarización está activa, cada frase se ordena cronológicamente, y se formatea como \texttt{[Speaker N]: texto}, produciendo una transcripción estructurada por hablante.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/Transcription System.png}
    \caption{Diagrama de flujo de el proceso de transcripción.}
    \label{fig:transcription_workflow}
\end{figure}
...
\paragraph{Enfoque mediante Claude Cowork}
  ...
  \begin{figure}[H]
      \centering
      \includegraphics[width=1\textwidth]{imagenes/requirements_intake_skill.png}
...
Con el objetivo de disponer de una alternativa de coste reducido e independiente de suscripciones comerciales, se ha desarrollado una aplicación Python que replica el proceso anterior. La aplicación está estructurada en módulos desacoplados y orquestada mediante LangChain \cite{langchain_2022}.
...
Una característica diferencial de la aplicación es la capacidad de refinamiento iterativo en cada fase. El usuario puede modificar el plan de investigación o el plan arquitectónico mediante instrucciones en lenguaje natural. El sistema mantiene un historial de conversación mediante la abstracción \texttt{MessagesPlaceholder} de LangChain \cite{langchain_2022}, lo que permite que el modelo tome en cuenta el contexto de intercambios anteriores al procesar cada solicitud de refinamiento. Este mecanismo implementa el principio \textit{Human in the Loop} definido en los objetivos del proyecto.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{imagenes/Requirements Intake System.png}
    \caption{Diagrama de flujo de el proceso generación de plan en la toma de requerimientos.}
    \label{fig:requirements_intake_workflow}
\end{figure}
...
\begin{itemize}
    ...
    \item Tareas asignadas al equipo de \textit{Colb.AI} con sus plazos correspondientes.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{imagenes/meeting_summaries_skill.png}
  \caption{Skill de seguimiento del cliente en Claude Cowork.}
  \label{fig:seguimiento_cliente_skill}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Flujo de la implementación propia del módulo de seguimiento del cliente: generación del resumen inicial y bucle iterativo de refinamiento con \textit{Human in the Loop}.}
...
Este capítulo describe la arquitectura de herramientas y asistentes que se ha configurado para implementar la metodología SDD en el día a día del equipo de desarrollo.
...
Para comprender plenamente cómo se pueden extender y configurar estas herramientas, es conveniente conocer la arquitectura interna sobre la que están construidas. Tanto ClaudeCode como OpenCode son aplicaciones de TypeScript que implementan un bucle de agente basado en el patrón ReAct (Reasoning + Acting) \cite{yao_react_2023} como paradigma estándar para la construcción de agentes basados en modelos de lenguaje.

El patrón ReAct estructura el comportamiento del agente en un ciclo iterativo de tres fases:

\textbf{Razonamiento (Thought):} El modelo recibe el estado actual del contexto y genera un razonamiento interno en el que planifica el siguiente paso. Este razonamiento es el mecanismo por el que el agente decide qué acción tomar.

\textbf{Acción (Action):} El modelo emite una llamada a herramienta estructurada (\textit{tool call}) especificando qué herramienta invocar y con qué parámetros. Las herramientas disponibles incluyen habitualmente la lectura de archivos, la escritura de código y la ejecución de comandos en terminal, aunque por supuesto dependen del sistema que se utilice.

\textbf{Observación (Observation):} Se ejecuta la acción sobre el sistema de archivos de la terminal y devuelve el resultado al modelo como nuevo mensaje en el historial. Este resultado se convierte en la entrada del siguiente ciclo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{imagenes/Ciclo_ReAct.png}
    \caption{Ciclo iterativo ReAct: el agente alterna entre Razonamiento, Acción y Observación hasta que no hay más herramientas que invocar.}
    \label{fig:ciclo_react}
\end{figure}

El flujo de ejecución que orquesta este bucle puede describirse de forma simplificada en tres pasos:

\begin{enumerate}
    \item Al iniciarse una sesión, el runtime construye el system prompt combinando las instrucciones globales del asistente con el contenido del archivo de configuración del proyecto (CLAUDE.md, AGENT.md o equivalente), además de definir el conjunto de herramientas disponibles mediante un esquema JSON (habitualmente).
...
\subsubsection{Similitud con ClaudeCode}
...
\midrule
\textbf{Proveedores LLM} & Exclusivo de modelos compatibles con la API de Anthropic (modelos Claude principalmente) & Más de 25 proveedores a través del SDK de Vercel AI (OpenAI, Google, Azure, Bedrock, Mistral, Groq, GitHub Copilot, entre otros) \\
...
\textbf{Framework técnico} & TypeScript estándar para la composición de servicios & Framework Effect para gestión de dependencias, errores y concurrencia \\
\bottomrule
\end{tabular}
\caption{Comparativa arquitectónica entre ClaudeCode y OpenCode.}
...
code-review-graph \cite{code_review_graph_2025} es una herramienta de código abierto que utiliza el parser de sintaxis Tree-Sitter~\cite{tree_sitter} para analizar el repositorio y construir un grafo de dependencias entre las distintas entidades de código (funciones, clases, módulos). Este grafo se almacena en una base de datos SQLite local y se expone al agente a través de un servidor MCP, proporcionando un mecanismo eficiente para navegar la estructura del código sin necesidad de leer todos los archivos del proyecto.

La integración de esta herramienta aborda directamente el problema de gestión de la ventana de contexto identificado en el apartado de riesgos (sección~\ref{sec:riesgo5}). Según los \textit{benchmarks} oficiales publicados en el repositorio de la herramienta~\cite{code_review_graph_2025}, evaluados sobre 6 repositorios reales de código abierto (Flask, FastAPI, Express, Gin, httpx y el propio code-review-graph), el contexto del grafo reduce el consumo de tokens respecto a proporcionar el repositorio completo en un rango de \textbf{38× a 528×}, con una mediana de $\sim$82×: una consulta típica requiere entre 1.986 y 3.465 tokens extraídos del grafo, frente a los 89.492--951.071 que supondría leer la totalidad de los archivos fuente del repositorio correspondiente. Las capacidades que expone al agente incluyen:
\begin{itemize}
...
En caso de que una de las anteriores no exista, al agente se le indica que ejecute las skills de "defining-constitution" y "defining-features", para popular estos archivos a través de un proceso interactivo de preguntas al usuario.
...
\begin{figure}[H]
    ...
    \caption{Diagrama de flujo de colbPowers.}
    \label{fig:colbPowers_workflow}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Generación de la especificación:} cuando el usuario confirma que la definición es correcta, el agente redacta el documento de especificación (design.md) y lo guarda en \texttt{docs/specs/}. Este documento es la fuente de verdad de la implementación y es consultado en cada paso posterior.
    ...
    \item \textbf{Ejecución iterativa con implementador y revisor:} el agente utiliza la herramienta de lista de tareas (\texttt{TodoWrite}) integrada en los asistentes CLI para ejecutar el plan de forma iterativa. El agente orquestador da la opción al desarrollador de usar \textbf{subagent-driven-development} o \textbf{inline-execution}: (1) La primera opción para cada tarea actúa un subagente implementador que escribe el código, y a continuación un subagente revisor que verifica que el código se ajusta a la especificación, sigue el plan y pasa los tests definidos. (2) La segunda opción utiliza al propio agente orquestador como implementador, y revisor, pero como al ser el mismo agente el que lo hace todo, estará mas sesgado por el contexto anterior, adicionalmente, mediante se llene su contexto, será mas probable que se salte alguno de los pasos del flujo de implementación. 
    \end{enumerate}
...
\paragraph{Limitaciones del diseño experimental.} Deben señalarse dos limitaciones estructurales.
\begin{enumerate}
    \item El tamaño muestral de $n=3$ repeticiones por condición limita la potencia estadística de los resultados e impide verificar los supuestos de normalidad del test $t$ de Welch; los resultados deben interpretarse como evidencia sugerente. 
    \item Tanto el diseño de la intervención (colbPowers) como la ejecución y evaluación de los experimentos son obra del mismo autor, en el contexto de la empresa para la que se desarrolla el sistema (\textit{Colb.AI}). Esta ausencia de evaluación ciega o independiente es una limitación metodológica reconocida, y los resultados deben interpretarse teniendo en cuenta un potencial sesgo de confirmación. 
    % \item La condición colbPowers incluye acceso al servidor MCP \textit{code-review-graph}, mientras que la condición baseline no dispone de él; dado que colbPowers puede operar sin este componente, no es posible aislar la contribución individual de la metodología SDD respecto a la del análisis estático proporcionado por \textit{code-review-graph}. Este experimento de control se propone como trabajo futuro.
\end{enumerate}
...
Se utiliza el test $t$ de Welch~\cite{welch1947} para comparar las medias de cada métrica entre condiciones, dado que no puede asumirse igualdad de varianzas entre grupos. Como estimador del tamaño del efecto se emplea la $g$ de Hedges~\cite{Hedges}, que corrige el sesgo de la $d$ de Cohen en muestras pequeñas. Se emplea el umbral estándar de $p < 0{,}05$ como indicativo de significación estadística; la $g$ cuantifica la magnitud solo cuando el $p$-value permite rechazar la hipótesis nula. Los umbrales de interpretación de $g$ son: trivial ($g < 0{,}2$), pequeño ($0{,}2 \leq g < 0{,}5$), mediano ($0{,}5 \leq g < 0{,}8$) y grande ($g \geq 0{,}8$)~\cite{cohen1988statistical}. Dado el tamaño muestral ($n=3$), todos los $p$-values deben considerarse indicativos, no confirmatorios.

\subsection{Caso 1: Aplicación Flutter}
\label{sec:caso1_confortmental}
El primer experimento se llevó a cabo sobre un repositorio real de \textit{Colb.AI}. Con tal de respetar las políticas de confidencialidad, la descripción del proyecto es lo suficientemente detallada como para entender el contexto, omitiendo ciertos detalles para anonimizarlo.
...
En ambos asistentes el paso de baseline a colbPowers multiplica el consumo de tokens ($\times$2,9 en OpenCode, $\times$3,8 en ClaudeCode). El número de tool calls crece moderadamente en OpenCode (69 → 92) y se mantiene similar en ClaudeCode (53 → 63). Las llamadas a \textit{code-review-graph} son esporádicas: la Ejecución~2 de OpenCode colbPowers registró 2 llamadas y la Ejecución~3 de ClaudeCode colbPowers registró 3; el resto, cero.

El desglose de tokens revela una diferencia estructural entre asistentes. En OpenCode los tokens de entrada no cacheados son significativos (media 0,98~M en baseline, 4,07~M en colbPowers), lo que refleja que cada subagente inicia un contexto propio sin aprovechar caché de sesiones anteriores. En ClaudeCode, en cambio, los tokens de entrada son prácticamente nulos (aprox.\ 0,00~M) y casi la totalidad del consumo corresponde a tokens de caché (3,63~M en baseline, 13,70~M en colbPowers).

En cuanto a la calidad del código, colbPowers produce mayor complejidad ciclomática que baseline en ambos asistentes (OpenCode: 3,37 frente a 2,67, $+$26\%; ClaudeCode: 3,48 frente a 2,98, $+$17\%), sin variación apreciable en el índice de mantenibilidad (OpenCode: 66,59 frente a 67,59; ClaudeCode: 66,52 frente a 66,05). El análisis estadístico y la interpretación de estos resultados se presentan en la Sección~\ref{sec:discusion_resultados}.
...
\begin{tabular}{l l r r r r r r r r}
\toprule
\textbf{Condición} & \textbf{Ejecución} & \textbf{Total (M)} & \textbf{Input (M)} & \textbf{Output (M)} & \textbf{Caché (M)} & \textbf{Tool calls} & \textbf{CRG} & \textbf{Comp. ciclom.} & \textbf{Mant. Index} \\
\midrule
\textbf{baseline} & Ejec.~1 & 1,59 & 0,97 & 0,01 & 0,59 & 36 & 0 & 4,67 & 44,09 \\
 & Ejec.~2 & 2,52 & 1,41 & 0,02 & 1,08 & 70 & 0 & 4,00 & 39,24 \\
 & Ejec.~3 & 1,53 & 0,38 & 0,02 & 1,13 & 49 & 0 & 4,25 & 41,30 \\
 & \textbf{Media} & \textbf{1,88} & \textbf{0,92} & \textbf{0,02} & \textbf{0,93} & \textbf{52} & \textbf{0} & \textbf{4,31} & \textbf{41,54} \\
\midrule
\textbf{colbPowers} & Ejec.~1 & 6,53 & 2,69 & 0,06 & 3,77 & 80 & 1 & 3,27 & 47,20 \\
 & Ejec.~2 & 10,22 & 6,28 & 0,07 & 3,84 & 86 & 1 & 2,97 & 48,66 \\
 & Ejec.~3 & 8,47 & 5,12 & 0,06 & 3,25 & 89 & 0 & 3,16 & 47,47 \\
 & \textbf{Media} & \textbf{8,41} & \textbf{4,70} & \textbf{0,06} & \textbf{3,62} & \textbf{85} & \textbf{0,7} & \textbf{3,13} & \textbf{47,78} \\
\bottomrule
\end{tabular}
}
\caption{Caso 2 --- OpenCode con DeepSeek V4-Flash. Caché = tokens leídos de caché.}
...
\textbf{Condición} & \textbf{Ejecución} & \textbf{Total (M)} & \textbf{Input (M)} & \textbf{Output (M)} & \textbf{Caché (M)} & \textbf{Tool calls} & \textbf{CRG} & \textbf{Comp. ciclom.} & \textbf{Mant. Index} \\
\midrule
\textbf{baseline} & Ejec.~1 & 6,12 & 0,00 & 0,05 & 6,00 & 90 & 0 & 2,50 & 49,92 \\
 & Ejec.~2 & 2,75 & 0,00 & 0,02 & 2,68 & 54 & 0 & 3,23 & 62,24 \\
 & Ejec.~3 & 2,37 & 0,00 & 0,04 & 2,27 & 44 & 0 & 2,83 & 58,76 \\
 & \textbf{Media} & \textbf{3,74} & \textbf{0,00} & \textbf{0,04} & \textbf{3,65} & \textbf{63} & \textbf{0} & \textbf{2,85} & \textbf{56,97} \\
\midrule
\textbf{colbPowers} & Ejec.~1 & 7,30 & 0,00 & 0,11 & 6,82 & 46 & 0 & 2,77 & 50,56 \\
 & Ejec.~2 & 6,28 & 0,00 & 0,09 & 5,93 & 44 & 1 & 3,65 & 67,18 \\
 & Ejec.~3 & 10,97 & 0,00 & 0,11 & 10,11 & 55 & 0 & 2,90 & 59,28 \\
 & \textbf{Media} & \textbf{8,19} & \textbf{0,00} & \textbf{0,10} & \textbf{7,62} & \textbf{48} & \textbf{0,3} & \textbf{3,11} & \textbf{59,01} \\
\bottomrule
\end{tabular}
}
\caption{Caso 2 --- ClaudeCode con Haiku~4.5. Caché = tokens leídos de caché.}
\label{tab:caso2_claudecode}
\end{table}

El desglose de tokens del Caso~2 confirma el mismo patrón estructural del Caso~1: en ClaudeCode el consumo se concentra en caché (3,65~M en baseline, 7,62~M en colbPowers) con tokens de entrada prácticamente nulos, mientras que en OpenCode los tokens de entrada son significativos (0,92~M en baseline, 4,70~M en colbPowers). A diferencia del Caso~1, en OpenCode la distribución entre input y caché es más equilibrada en la condición baseline (0,92~M frente a 0,93~M), consistente con que el proyecto parte de un repositorio vacío. El sobrecoste de colbPowers es $\times$4,5 en OpenCode y $\times$2,2 en ClaudeCode. En ClaudeCode el orquestador realiza menos tool calls bajo colbPowers (48) que bajo baseline (63), reflejo de su mayor delegación a subagentes.

En cuanto a la calidad del código, en OpenCode colbPowers genera código de menor complejidad ciclomática (3,13 frente a 4,31, $-$27\%) y mayor índice de mantenibilidad (47,78 frente a 41,54, $+$15\%). En ClaudeCode los cambios son menores: complejidad 2,85 → 3,11 y mantenibilidad 56,97 → 59,01 ($+$3,6\%). Destaca que en la condición baseline ClaudeCode produce código con un índice de mantenibilidad notablemente mayor que OpenCode (56,97 frente a 41,54). El análisis estadístico y la interpretación de estos resultados se presentan en la Sección~\ref{sec:discusion_resultados}.
...
El sobrecoste de tokens de colbPowers es el hallazgo más robusto del experimento: presente en todas las condiciones evaluadas, con tamaño de efecto grande en todas ellas. En el Caso~1, el factor respecto al baseline es $\times$2,9 para OpenCode ($p=0{,}032$, $g=+3{,}21$) y $\times$3,8 para ClaudeCode ($p=0{,}005$, $g=+3{,}81$); en el Caso~2, $\times$4,5 para OpenCode ($p=0{,}019$, $g=+3{,}84$) y $\times$2,2 para ClaudeCode ($p=0{,}077$, $g=+1{,}56$). La única excepción es el Caso~2 con ClaudeCode, donde la diferencia no alcanza significación estadística con $n=3$, aunque el tamaño de efecto es igualmente grande. Agrupando ambos casos, el sobrecoste es altamente significativo tanto para OpenCode ($p<0{,}001$, $g=+4{,}03$) como para ClaudeCode ($p=0{,}004$, $g=+2{,}23$). Este coste refleja el overhead estructural del flujo SDD: generación de documentación (constitution, features, spec, plan), delegación a una media de 9 a 14 subagentes por sesión y ejecución de revisiones sistemáticas. La Figura~\ref{fig:tokens_totales} muestra el consumo por experimento, asistente y condición.
...
El desglose por tipo de token revela una diferencia estructural entre los dos asistentes. En OpenCode los tokens de entrada no cacheados son significativos (media 0,98~M en baseline y 4,07~M en colbPowers para el Caso~1), lo que indica que cada subagente inicia un contexto propio sin aprovechar la caché de sesiones anteriores. En ClaudeCode, en cambio, los tokens de entrada son prácticamente nulos y casi la totalidad del consumo corresponde a tokens de caché (3,63~M en baseline y 13,70~M en colbPowers para el Caso~1); la causa de esta diferencia de comportamiento no se ha podido determinar en el alcance de este trabajo. En cualquier caso, esta asimetría hace que la comparación del total de tokens entre asistentes no sea directa: ClaudeCode acumula más tokens totales que OpenCode bajo colbPowers en el Caso~1 (14,33~M frente a 9,38~M, $p=0{,}039$, $g=-1{,}98$), pero esto refleja principalmente mayor reutilización de caché, no mayor trabajo efectivo. En baseline, en cambio, los totales de ambos asistentes son comparables (3,26~M frente a 3,74~M, diferencia no significativa).

Este sobrecoste no debe interpretarse en aislamiento. colbPowers genera como subproducto documentación estructurada (constitución del proyecto, especificación de cambios, plan de implementación) que persiste en el repositorio, y su énfasis en criterios de aceptación explícitos propicia la generación de tests como parte natural del flujo. En un proyecto con horizonte de vida largo, estos beneficios transcienden el consumo inmediato de tokens.

\subsubsection{Métricas de calidad}

Las métricas de calidad ---complejidad ciclomática e índice de mantenibilidad--- se calcularon para ambos casos con herramientas de análisis estático adaptadas al lenguaje: DCM para Flutter/Dart (Caso~1) y radon para Python (Caso~2). Se presentan primero los resultados del Caso~2 y a continuación los del Caso~1, dado que el patrón difiere cualitativamente entre ellos.

% ── Caso 2 ──────────────────────────────────────────────────────────────────

\paragraph{Caso~2 (ClassificationVisualization) --- Baseline vs.\ colbPowers.}
La Figura~\ref{fig:calidad_codigo_caso2} muestra las métricas medias por asistente y condición. En OpenCode, colbPowers produce una reducción de la complejidad ciclomática (de 4,31 a 3,13, $-$27\%; $p=0{,}014$, efecto grande) y un aumento del índice de mantenibilidad (de 41,54 a 47,78, $+$15\%; $p=0{,}037$, efecto grande). En ClaudeCode las diferencias no alcanzan significación estadística: la complejidad ciclomática pasa de 2,85 a 3,11 ($p=0{,}508$) y el índice de mantenibilidad de 56,97 a 59,01 ($p=0{,}754$), por lo que este test no ha podido detectar un efecto estadísticamente significativo; con $n=3$ el efecto podría existir sin ser detectable.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/comparisons/01_calidad_codigo_caso2.png}
    \caption{Métricas de calidad del código --- Caso~2 (ClassificationVisualization, radon/Python). Medias por asistente y condición.}
    \label{fig:calidad_codigo_caso2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graficos/comparisons/08_delta_calidad_caso2.png}
    \caption{Efecto de colbPowers sobre la calidad del código en el Caso~2 (delta = colbPowers $-$ baseline; verde: mejora, rojo: empeora).}
    \label{fig:delta_calidad_caso2}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_cpbl_bars_Opencode_-_ClassificationVisu.png}
    \caption{colbPowers vs.\ baseline, OpenCode, Caso~2. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo. (ns: no significativo, $p>0{,}05$).}
...
\begin{figure}[H]
    ...
    \caption{colbPowers vs.\ baseline, ClaudeCode, Caso~2. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo. (ns: no significativo, $p>0{,}05$).}
    \label{fig:ttest_cpbl_claudecode_classvis}
\end{figure}

\paragraph{Caso~2 --- OpenCode vs.\ ClaudeCode.}
En baseline, ClaudeCode genera código con menor complejidad ciclomática (2,85 vs.\ 4,31; $p=0{,}007$, efecto grande) y mayor índice de mantenibilidad (56,97 vs.\ 41,54; $p=0{,}039$, efecto grande). Con colbPowers activo, las diferencias se reducen considerablemente ---la complejidad converge a valores casi idénticos (3,13 OpenCode, 3,11 ClaudeCode)--- y los $p$-values no alcanzan significación estadística, excepto en las tool calls del orquestador, donde ClaudeCode realiza menos llamadas que OpenCode ($p=0{,}001$, $g=+5{,}58$), reflejo de su mayor delegación a subagentes.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_occc_bars_baseline_-_ClassificationVisu.png}
    \caption{OpenCode vs.\ ClaudeCode, baseline, Caso~2. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo. (ns: no significativo, $p>0{,}05$).}
...
\begin{figure}[H]
    ...
    \caption{OpenCode vs.\ ClaudeCode, colbPowers, Caso~2. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo. (ns: no significativo, $p>0{,}05$).}
    \label{fig:ttest_occc_colbpowers_classvis}
\end{figure}

% ── Caso 1 ──────────────────────────────────────────────────────────────────

\paragraph{Caso~1 (Repo Existente) --- Baseline vs.\ colbPowers.}
En el Caso~1 el patrón se invierte respecto al Caso~2: colbPowers produce mayor complejidad ciclomática que baseline tanto en OpenCode (3,37 vs.\ 2,67, $+$26\%; $p=0{,}029$, $g=+2{,}42$) ---diferencia estadísticamente significativa--- como en ClaudeCode (3,48 vs.\ 2,98, $+$17\%; $p=0{,}208$, $g=+1{,}07$), aunque en este último sin soporte estadístico. El índice de mantenibilidad no muestra diferencia apreciable en ningún asistente (OpenCode: $p=0{,}646$, $g=-0{,}32$; ClaudeCode: $p=0{,}873$, $g=+0{,}11$). Este resultado no confirma la hipótesis de que \textit{code-review-graph} proporcionaría un contexto de dependencias superior: las llamadas al servidor MCP fueron prácticamente nulas en todas las ejecuciones, lo que privó al flujo SDD de la ventaja de exploración que habría justificado su overhead.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/comparisons/01_calidad_codigo_caso1.png}
    \caption{Métricas de calidad del código --- Caso~1 (Repo Existente, DCM/Flutter). Medias por asistente y condición.}
    \label{fig:calidad_codigo_caso1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{graficos/comparisons/08_delta_calidad_caso1.png}
    \caption{Efecto de colbPowers sobre la calidad del código en el Caso~1 (delta = colbPowers $-$ baseline; verde: mejora, rojo: empeora).}
    \label{fig:delta_calidad_caso1}
\end{figure}

\paragraph{Caso~1 --- OpenCode vs.\ ClaudeCode.}
En el Caso~1 las diferencias de calidad entre asistentes son menos pronunciadas que en el Caso~2 y ninguna alcanza significación estadística. En baseline, la complejidad ciclomática es similar entre asistentes ($p=0{,}129$, $g=-1{,}26$) y el índice de mantenibilidad también ($p=0{,}596$, $g=+0{,}38$). Con colbPowers la brecha es igualmente irrelevante estadísticamente (complejidad: $p=0{,}746$, $g=-0{,}23$; mantenibilidad: $p=0{,}979$, $g=+0{,}02$), lo que sugiere que en tareas de modificación incremental la elección del asistente tiene menor impacto en la calidad estructural del código producido.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_occc_bars_baseline_-_Repo_Existente.png}
    \caption{OpenCode vs.\ ClaudeCode, baseline, Caso~1. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo.}
...
\begin{figure}[H]
    ...
    \caption{OpenCode vs.\ ClaudeCode, colbPowers, Caso~1. Barras: media $\pm$ SD. Anotación: $g$ de Hedges y $p$-value indicativo.}
...
Este trabajo ha abordado un problema relevante en el desarrollo de software asistido por inteligencia artificial: la tendencia del paradigma \textit{Code-Driven} a generar código de baja calidad estructural cuando no existe una fase previa de especificación y planificación. La propuesta central del proyecto, el plugin colbPowers y el flujo \textit{Spec-Driven Development} (SDD), ha demostrado ser técnicamente viable y evaluable dentro del entorno de trabajo de \textit{Colb.AI}, donde actualmente se utiliza este sistema. 

\subsubsection*{Cumplimiento de requisitos}

Se van a mostrar los requisitos funcionales y no funcionales definidos anteriormente en \ref{sec:req_funcionales} y \ref{sec:req_no_funcionales}, se iterará sobre cada uno de estos justificando si se han conseguido cubrir adecuadamente.

\paragraph{Requisitos funcionales}

\subparagraph{El sistema debe permitir la ingesta y gestión de reglas fundamentales del proyecto (pila tecnológica, estándares de código) y mantener un registro actualizado de las características a desarrollar.}
Este requisito queda cubierto mediante los archivos \texttt{constitution.md} y \texttt{features.md}, que actúan como fuente de verdad del proyecto. \texttt{constitution.md} centraliza la pila tecnológica y los estándares de código del proyecto; \texttt{features.md} mantiene el registro actualizado de funcionalidades pendientes, en progreso y completadas. Adicionalmente, los documentos \texttt{plan.md} y \texttt{spec.md} generados durante el flujo SDD permiten consultar qué apartados han sido ya implementados y de qué forma concreta se llevó a cabo su implementación. Todos estos archivos se actualizan de forma iterativa durante el flujo SDD y persisten versionados en el repositorio Git del proyecto.

\subparagraph{El sistema debe ser capaz de interpretar funcionalidades de alto nivel y desglosarlas en un plan de tareas granulares y estructuradas.}
Este requisito se implementa en la fase de planificación del flujo SDD, donde el agente orquestador genera un plan de tareas granulares (\texttt{spec.md} y \texttt{plan.md}) a partir de una funcionalidad de alto nivel definida en por el usuario, antes de delegar la implementación a los subagentes especializados. Este mecanismo ha sido validado en los dos casos experimentales del Capítulo \ref{sec:evaluacion_resultados}.

\subparagraph{El sistema debe incluir un mecanismo de auditoría autónoma que contraste el código fuente generado con las especificaciones documentadas, para ver si existe algún conflicto con las documentación y el código generado.}
Este requisito queda cubierto por el subagente revisor de coherencia con especificaciones, que contrasta el código generado con los documentos de contexto (\texttt{constitution.md}, \texttt{features.md}) e identifica posibles conflictos o desviaciones. Como se discute en las limitaciones, este mecanismo es no determinista, puesto que está basado en \textit{prompts}, y en uno de los experimentos el orquestador omitió su invocación, lo que representa la principal limitación en el cumplimiento de este requisito.

\subparagraph{El sistema debe incluir un módulo de procesamiento de transcripciones capaz de generar planes de implementación estructurados a partir de reuniones con el cliente, explorando automáticamente los \textit{frameworks} y tecnologías relevantes mediante búsqueda semántica en la web.}
Este requisito queda cubierto por el módulo de Toma de Requerimientos (Sección~\ref{sec:toma_requerimientos_cap}), que procesa la transcripción de una reunión inicial con el cliente y genera un plan de implementación estructurado. El módulo incluye búsqueda web automática para explorar \textit{frameworks} y tecnologías relevantes según el contexto del proyecto descrito en la reunión.

\subparagraph{El sistema debe incluir un módulo de generación de documentación de seguimiento post-reunión con el formato específico de \textit{Colb.AI}, a partir de la transcripción de reuniones periódicas con el cliente.}
Este requisito queda cubierto por el módulo de Seguimiento del Cliente (Sección~\ref{sec:seguimiento_cliente}), que genera documentación post-reunión en el formato específico de \textit{Colb.AI} a partir de la transcripción de reuniones periódicas. El módulo produce los artefactos de seguimiento utilizados actualmente en los proyectos de la empresa.

\paragraph{Requisitos no funcionales}

\subparagraph{Transparencia y Trazabilidad: Todo el estado, la memoria y las normativas manejadas por los agentes deben persistir en formatos de texto en un sistema de control de versiones.}
Todos los artefactos del sistema ---\texttt{constitution.md}, \texttt{features.md}, los documentos generados por los módulos de transcripción y los propios \textit{prompts} y \textit{skills}--- son archivos de texto plano versionados en Git. Esto garantiza que cualquier cambio en el estado del sistema sea auditable y reversible, cumpliendo plenamente este requisito.

\subparagraph{Integración con asistentes CLI: El plugin debe ser compatible con ClaudeCode y OpenCode, utilizando sus mecanismos nativos de extensibilidad (MCP, hooks, skills) para actuar directamente sobre los archivos locales del proyecto.}
El plugin colbPowers ha sido validado experimentalmente sobre los dos asistentes CLI objetivo: ClaudeCode y OpenCode. Los experimentos del Capítulo \ref{sec:evaluacion_resultados} confirman la compatibilidad efectiva con ambos asistentes.

\subparagraph{Modularidad: El diseño de los roles (\textit{prompts} y \textit{skills}) debe ser lo suficientemente modular para permitir futuras expansiones.}
El sistema está estructurado en componentes independientes: subagentes especializados (implementador, revisor de código, revisor de coherencia), módulos de transcripción y documentación, y herramientas de apoyo como \textit{code-review-graph}. Esta separación de responsabilidades permite añadir, modificar o sustituir componentes sin afectar al resto del sistema. Las líneas de trabajo futuro descritas en la sección siguiente ilustran la viabilidad de extensiones concretas sobre esta arquitectura modular.

\subparagraph{Reducción del \textit{vendor lock-in}.}
Aunque este aspecto no fue formulado explícitamente como requisito no funcional, sino como un riesgo identificado en la Sección~\ref{sec:obstaculos_riesgos}, merece destacarse que ha sido abordado de forma muy satisfactoria a lo largo del proyecto. Para cada módulo o componente con dependencia de una plataforma comercial se ha desarrollado una alternativa propia o de código abierto: los módulos de transcripción disponen de implementaciones independientes de Claude Cowork; el flujo SDD puede ejecutarse tanto sobre ClaudeCode como sobre OpenCode; y las herramientas de análisis estático empleadas (radon, DCM) son de código abierto. Esta cobertura garantiza que el sistema pueda desplegarse y mantenerse sin quedar a la merced de un único proveedor externo.
...
La evaluación experimental produce resultados mixtos. Dado el tamaño muestral ($n=3$ por condición), se emplea la $g$ de Hedges como estimador principal del tamaño del efecto; el $p$-value del test $t$ de Welch indica si puede rechazarse la hipótesis nula de ausencia de efecto, y la $g$ cuantifica su magnitud solo cuando el $p$-value permite ese rechazo. El hallazgo principal es la mejora estadísticamente significativa y de efecto grande en las métricas de calidad del código en el Caso~2 (proyecto generado desde cero): colbPowers reduce la complejidad ciclomática un 27\% y mejora el índice de mantenibilidad un 15\% en OpenCode ($p=0{,}014$ y $p=0{,}037$ respectivamente). Con ClaudeCode, que de base produce código de mayor calidad, los cambios fueron mucho menos notables ---un aumento del 9\% en complejidad ciclomática y una variación del 3\% en mantenibilidad--- y no alcanzan significación estadística ($p=0{,}508$ y $p=0{,}754$), por lo que este test no ha podido rechazar la hipótesis nula; el efecto podría existir sin ser detectable con $n=3$. En el Caso~1 (eliminación de funcionalidades complejas sobre un repositorio existente), el patrón se invierte: colbPowers produce código de mayor complejidad ciclomática ($+$26\% en OpenCode, $p=0{,}029$, $g=+2{,}42$; $+$17\% en ClaudeCode, $p=0{,}208$, $g=+1{,}07$), con efecto estadísticamente significativo solo en OpenCode, sin mejora neta en mantenibilidad en ningún asistente (OpenCode: $p=0{,}646$; ClaudeCode: $p=0{,}873$). Uno de los facores destacables es que parece ser que los agentes no invocaron \textit{code-review-graph} de forma efectiva, privando al flujo SDD del contexto de dependencias.

Un resultado secundario con implicaciones prácticas directas es la convergencia de calidad entre asistentes: con colbPowers activo, las diferencias de calidad entre OpenCode y ClaudeCode no alcanzan significación estadística, y la $g$ de Hedges cae a valores próximos a cero en ambas métricas, lo que indica que el flujo SDD homogeneiza la calidad del código con independencia del asistente y permite a un equipo combinar herramientas distintas sin comprometer la consistencia del producto.

El coste del sistema es real y cuantificado: colbPowers multiplica el consumo de tokens por un factor de entre 2{,}2x y 4{,}5x respecto al baseline, con tamaños de efecto grandes en todas las combinaciones evaluadas. Este sobrecoste es constante e independiente del resultado: se produce en todos los experimentos, pero la mejora de calidad que lo justificaría solo ha podido confirmarse estadísticamente en el Caso~2 con OpenCode. En el Caso~1, el overhead no se tradujo en mejoras de calidad, lo que indica que la relación entre coste computacional y beneficio estructural depende del tipo de tarea abordada.

\subsubsection*{Limitaciones}

El diseño experimental con $n=3$ repeticiones por condición tiene potencia estadística limitada e impide verificar los supuestos de normalidad requeridos por el test $t$ de Welch. Los resultados deben considerarse evidencia sugerente, no concluyente, y deben ser confirmados con tamaños muestrales mayores ($n \geq 5$) en trabajos futuros. Adicionalmente, las herramientas de análisis estático empleadas difieren entre casos (radon para Python y DCM para Flutter/Dart), lo que implica que las métricas de calidad no son directamente comparables entre experimentos, aunque sí lo son dentro de cada caso entre condiciones. Por último, el mecanismo de garantía del flujo SDD es no determinista, en un experimento el agente orquestador omitió la invocación del subagente revisor.

\subsubsection*{Valoración global}

Los resultados respecto a la hipótesis principal ---que estructurar el desarrollo mediante un flujo SDD mejora la calidad del código generado--- son mixtos: el flujo mostró mejoras estadísticamente significativas únicamente en el Caso~2 con OpenCode, mientras que en el Caso~1 produjo un aumento de la complejidad ciclomática y no aportó mejoras en mantenibilidad en ningún asistente. Estos resultados, aun sin confirmar la hipótesis de forma general, justifican continuar investigando el enfoque, ampliar el tamaño muestral y extender las métricas de evaluación a otros lenguajes y tipos de proyecto. La contribución técnica del TFG (un plugin funcional, dos módulos de apoyo y un conjunto de datos experimentales cuantificables sobre el efecto del flujo SDD) proporciona una base concreta para este trabajo futuro.

Cabe señalar, no obstante, que los resultados experimentales mixtos contrastan con la experiencia de uso real del sistema en \textit{Colb.AI}, donde la valoración ha sido positiva desde su adopción. Esta divergencia puede explicarse por dos factores no controlados en el experimento: por un lado, los modelos empleados en producción son en general más capaces que los utilizados en las ejecuciones experimentales; por otro, es posible que el entorno de experimentación no haya replicado con suficiente fidelidad las condiciones reales de uso, en particular la calidad y exhaustividad de los documentos de contexto proporcionados. Ambas hipótesis presentan líneas de análisis relevantes para trabajos futuros.
...
\begin{itemize}
    \item \textbf{colbPowers con y sin code-review-graph}, para aislar el efecto del servidor MCP de forma independiente al resto del flujo. Aun que se ha observado que el servidor fue poco utilizado, un experimento controlado podría cuantificar su impacto en la calidad del código y en el consumo de tokens.
    ...
    \item \textbf{Efecto del modelo subyacente sobre los resultados}, para contrastar si la divergencia entre los resultados experimentales y la experiencia de uso real en \textit{Colb.AI} se debe a la mayor capacidad de los modelos empleados en producción. Esto requeriría repetir los experimentos con los mismos modelos utilizados en el entorno real y comparar los efectos obtenidos.
    \item \textbf{Validación del entorno experimental}, para evaluar en qué medida las condiciones del experimento replican fielmente el uso real del sistema. En particular, convendría analizar si la calidad y exhaustividad de los documentos de contexto (\texttt{constitution.md}, \texttt{features.md}) proporcionados en los experimentos es comparable a la de los proyectos reales de la empresa, dado que la efectividad del flujo SDD depende directamente de la calidad de dichos documentos.
\end{itemize}
...
\section{Tablas completas análisis estadístico}
\label{sec:anexo_ttest}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_cpbl_table.png}
    \caption{Análisis estadístico: colbPowers vs.\ baseline. Un $p < 0{,}05$ permite rechazar la hipótesis nula; un $p$ alto no descarta el efecto, solo que el test no lo ha detectado. La $g$ de Hedges cuantifica el tamaño del efecto.}
...
\begin{figure}[H]
    ...
    \caption{Análisis estadístico: OpenCode vs.\ ClaudeCode. Un $p < 0{,}05$ permite rechazar la hipótesis nula; un $p$ alto no descarta el efecto, solo que el test no lo ha detectado. La $g$ de Hedges cuantifica el tamaño del efecto.}
...
\begin{figure}[H]
    ...
    \caption{Dispersión: tokens totales vs.\ métricas de calidad del código en ambos experimentos. Fila superior: Repo Existente (DCM); fila inferior: ClassificationVisualization (radon). Cada punto representa una sesión individual.}
