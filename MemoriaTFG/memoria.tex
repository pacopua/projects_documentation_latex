...
\hline
\texttt{baseline} & Herramientas nativas CLI & Solo Read, Write, Edit, Bash, Grep. Sin plugins ni servidores MCP adicionales. Actúa como línea base. \\
\hline
\texttt{colbPowers} & Nativas + colbPowers + code-review-graph & El agente sigue el flujo SDD de colbPowers con acceso al servidor MCP de code-review-graph. La tarea sigue el ciclo: constitution $\rightarrow$ features $\rightarrow$ spec $\rightarrow$ plan $\rightarrow$ execute $\rightarrow$ review. \\
\hline
\end{tabular}
...
\hline
\textbf{Condición} & \textbf{Ejecución} & \textbf{Total (M)} & \textbf{Input (M)} & \textbf{Output (M)} & \textbf{Caché (M)} & \textbf{Tool calls} & \textbf{CRG} \\
\hline
baseline & Ejec.~1 & 3,67 & 1,15 & 0,02 & 2,49 & 74 & 0 \\
 & Ejec.~2 & 2,76 & 0,89 & 0,02 & 1,85 & 68 & 0 \\
 & Ejec.~3 & 3,37 & 0,91 & 0,02 & 2,43 & 66 & 0 \\
 & \textbf{Media} & \textbf{3,26} & \textbf{0,98} & \textbf{0,02} & \textbf{2,26} & \textbf{69} & \textbf{0} \\
\hline
colbPowers & Ejec.~1 & 10,64 & 5,40 & 0,07 & 5,15 & 108 & 0 \\
 & Ejec.~2 & 10,55 & 4,18 & 0,07 & 6,27 & 71 & 2 \\
 & Ejec.~3 & 6,95 & 2,61 & 0,05 & 4,27 & 98 & 0 \\
 & \textbf{Media} & \textbf{9,38} & \textbf{4,07} & \textbf{0,06} & \textbf{5,23} & \textbf{92} & \textbf{0,7} \\
\hline
\end{tabular}
}
\caption{Caso 1 --- OpenCode con DeepSeek V4-Flash. Caché = tokens leídos de caché.}
...
baseline & Ejec.~1 & 6,64 & 0,01 & 0,04 & 6,50 & 76 & 0 \\
 ...
 & \textbf{Media} & \textbf{3,74} & \textbf{0,00} & \textbf{0,03} & \textbf{3,63} & \textbf{53} & \textbf{0} \\
\hline
colbPowers & Ejec.~1 & 12,23 & 0,00 & 0,11 & 11,66 & 56 & 0 \\
 & Ejec.~2 & 14,91 & 0,00 & 0,10 & 14,28 & 56 & 0 \\
 & Ejec.~3 & 15,85 & 0,00 & 0,11 & 15,15 & 76 & 3 \\
 & \textbf{Media} & \textbf{14,33} & \textbf{0,00} & \textbf{0,11} & \textbf{13,70} & \textbf{63} & \textbf{1,0} \\
\hline
\end{tabular}
}
\caption{Caso 1 --- ClaudeCode con Haiku~4.5. Caché = tokens leídos de caché.}
...
Este caso es especialmente relevante para evaluar colbPowers, ya que puede ejecutar el ciclo completo de la metodología SDD: primero se generan de forma interactiva con el usuario los documentos de constitución y \textit{features} del proyecto, y a continuación se genera la especificación técnica de la funcionalidad a implementar. En todo momento el proceso sigue el principio de \textit{Human-in-the-Loop}: el usuario participa en cada etapa y puede controlar el flujo. Para todas las ejecuciones del Caso 2 se han calculado métricas de calidad del código generado.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{graficos/01_calidad_codigo.png}
    \caption{Comparación de métricas de calidad en el Caso 2 (visualizador de clasificadores en Python). colbPowers reduce la complejidad ciclomática y mejora el índice de mantenibilidad, especialmente en OpenCode (CC: 4,31 → 3,13; MI: 41,54 → 47,78).}
    \label{fig:calidad_codigo}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{graficos/02_tokens_totales.png}
    \caption{Consumo total de tokens (millones) en ambos casos de estudio. El incremento de colbPowers respecto a baseline oscila entre 2,2x (ClaudeCode) y 4,5x (OpenCode), reflejando el overhead de documentación y delegación a subagentes.}
    \label{fig:tokens_totales}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{graficos/08_delta_calidad.png}
    \caption{Efecto del plugin colbPowers en la calidad del código (delta = plugin - baseline). En OpenCode, colbPowers reduce la complejidad ciclomática en 1,172 puntos (mejora, verde) e incrementa el índice de mantenibilidad en 6,233 puntos. En ClaudeCode los cambios son menores pero favorables. Este gráfico evidencia que a pesar del overhead de tokens, colbPowers genera código cualitativamente superior.}
