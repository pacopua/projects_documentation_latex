...
Las métricas de calidad de código del Caso~1, calculadas con DCM sobre el código modificado, no confirman la hipótesis de partida. En OpenCode, colbPowers produce código de \emph{mayor} complejidad ciclomática (3,37 frente a 2,67 del baseline, $+$26\%; $p=0{,}029$, $g=+2{,}42$), diferencia estadísticamente significativa y de magnitud grande, mientras que el índice de mantenibilidad no muestra variación apreciable (66,59 frente a 67,59, $-$1\%; $p=0{,}646$, $g=-0{,}32$). En ClaudeCode el incremento de complejidad ($+$17\%; $p=0{,}208$, $g=+1{,}07$) no alcanza significación estadística, y el índice de mantenibilidad permanece prácticamente invariante (66,52 frente a 66,05; $p=0{,}873$, $g=+0{,}11$).
...
El desglose de tokens del Caso~2 confirma el mismo patrón estructural descrito para el Caso~1: en ClaudeCode los tokens de entrada son prácticamente nulos en ambas condiciones (aprox.\ 0,00~M) y el consumo se concentra en tokens de caché (3,65~M en baseline, 7,62~M en colbPowers), mientras que en OpenCode los tokens de entrada son significativos (media 0,92~M en baseline, 4,70~M en colbPowers). Una diferencia notable respecto al Caso~1 es que en OpenCode la condición baseline presenta aquí una distribución más equilibrada entre input y caché (0,92~M frente a 0,93~M), en contraste con el Caso~1 donde la caché superaba claramente a los tokens de entrada (2,26~M frente a 0,98~M). Esta diferencia es consistente con que en el Caso~2 el proyecto parte de un repositorio vacío: el prefijo cacheado por los subagentes es inicialmente más reducido, por lo que una mayor fracción del contexto se transmite como tokens de entrada no cacheados.

En el segundo caso el sobrecoste de tokens de colbPowers es también considerable (x4,5 para OpenCode, x2,2 para ClaudeCode). En ClaudeCode el orquestador realiza menos tool calls en colbPowers (48) que en baseline (63), lo que confirma que delega la mayor parte del trabajo a subagentes. Las métricas de calidad muestran un patrón diferenciado según el asistente: en OpenCode, colbPowers genera código de menor complejidad ciclomática (3,13 frente a 4,31, una reducción del 27\%) y mayor índice de mantenibilidad (47,78 frente a 41,54, un incremento del 15\%), diferencias que resultan estadísticamente significativas. En ClaudeCode los cambios son mucho menores ---complejidad 3,11 (colbPowers) frente a 2,85 (baseline), mantenibilidad 59,01 frente a 56,97 ($+$3,6\%)--- y no alcanzan significación estadística ($p=0{,}508$ y $p=0{,}754$ respectivamente), por lo que no puede afirmarse la existencia de un efecto real. Destaca que ClaudeCode produce código con un índice de mantenibilidad notablemente mayor que OpenCode en la condición baseline (56,97 frente a 41,54, diferencia estadísticamente significativa con $p=0{,}039$), lo que sugiere una diferencia cualitativa entre los dos asistentes más allá del consumo de recursos.
...
\textit{Análisis estadístico.} Se utiliza el test $t$ de Welch \cite{welch1947} para comparar las medias de cada métrica entre condiciones, en lugar del $t$ de Student, dado que no se puede asumir igualdad de varianzas entre grupos. Como estimador del tamaño del efecto se emplea la $g$ de Hedges \cite{Hedges}, que corrige el sesgo de la $d$ de Cohen en muestras pequeñas.

Se emplea el umbral estándar de $p<0{,}05$ como indicativo de que la diferencia observada es estadísticamente significativa. El $p$-value determina si existe un efecto real; la $g$ de Hedges mide su magnitud una vez confirmada la significación. Los umbrales de interpretación de $g$ son: trivial ($g<0{,}2$), pequeño ($0{,}2 \leq g < 0{,}5$), mediano ($0{,}5 \leq g < 0{,}8$) y grande ($g \geq 0{,}8$) \cite{cohen1988statistical}. Cuando el $p$-value no es significativo ($p \geq 0{,}05$), el valor de $g$ no constituye evidencia fiable de un efecto real, independientemente de su magnitud.
...
La Figura~\ref{fig:calidad_codigo_caso2} muestra las métricas medias por asistente y condición. En OpenCode, colbPowers produce una reducción de la complejidad ciclomática (de 4,31 a 3,13, $-$27\%) y un aumento del índice de mantenibilidad (de 41,54 a 47,78, $+$15\%). En ClaudeCode el efecto es menos pronunciado: la complejidad ciclomática aumenta marginalmente (de 2,85 a 3,11) mientras que el índice de mantenibilidad mejora de 56,97 a 59,01 ($+$3,6\%).
...
\textit{Análisis estadístico.} En OpenCode la mejora con colbPowers presenta un tamaño de efecto grande tanto para la complejidad ciclomática como para el índice de mantenibilidad, con $p$-values indicativos de $p=0{,}014$ y $p=0{,}037$ respectivamente. En ClaudeCode, donde la baseline ya produce código de alta calidad, las diferencias observadas no alcanzan significación estadística con $n=3$ (complejidad: $p=0{,}508$; mantenibilidad: $p=0{,}754$), por lo que no puede afirmarse la existencia de un efecto real.

\paragraph{Caso~2 --- OpenCode vs.\ ClaudeCode.}
En la condición baseline, ClaudeCode genera código con menor complejidad ciclomática (2,85 vs.\ 4,31) y mayor índice de mantenibilidad (56,97 vs.\ 41,54), lo que sugiere que Haiku~4.5 produce soluciones más legibles de forma natural. Con colbPowers activo, las diferencias se reducen considerablemente: la complejidad converge a valores casi idénticos (3,13 OpenCode, 3,11 ClaudeCode), lo que sugiere que el flujo SDD homogeneiza la calidad con independencia del asistente.
...
\textit{Análisis estadístico.} En baseline las diferencias entre asistentes presentan efecto grande, con $p$-values indicativos de $p=0{,}007$ (complejidad) y $p=0{,}039$ (mantenibilidad). Con colbPowers, tenemos valores $p$ demasiado altos como para plantear la existencia de un efecto, excepto en el caso de las tool calls, donde ClaudeCode realiza menos llamadas que OpenCode ($p=0{,}001$) con un efecto considerable de $g=5{,}58$.
% ── Caso 1 ──────────────────────────────────────────────────────────────────
...
Las Figuras~\ref{fig:ttest_cpbl_oc_cm} y~\ref{fig:ttest_cpbl_cc_cm} (presentadas en la sección de análisis de consumo) muestran los paneles completos con barras de error (SD) y la $g$ de Hedges para cada comparación.
