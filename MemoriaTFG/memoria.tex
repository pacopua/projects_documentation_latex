...
{\textbf{Abstract}}
\end{center}
The present project consists of the design and application of an intelligent assistance system, with the objective of automating the various stages in the development of software projects, using mainly the orchestration of generative Artificial Intelligence tools. Placing emphasis on the interaction with the human developer, following a philosophy of \textit{Human in the loop}, and the generation of documentation to guide the process. This methodology looks to reduce the quality loss typically caused by the use of AI in the development of these projects. This final degree project is applied and developed for the working environment at the software consulting company \textit{Colb.AI}.
\vfill
\hspace{0pt}
}
...
{\textbf{Resumen}}
\end{center}
El presente proyecto engloba el diseño y aplicación de un sistema para la automatización de las diversas etapas en el desarrollo de proyectos de \textit{software}, utilizando principalmente la orquestación de herramientas de Inteligencia Artificial generativa. Haciendo un especial énfasis en la interacción de estos sistemas de automatización con el desarrollador, siguiendo una filosofía de \textit{Human in the loop}, y la generación de documentación para guiar el proceso. Esta metodología busca reducir la pérdida de calidad típicamente resultante del uso de IA en el desarrollo de los proyectos. Este trabajo de final de grado se aplica y desarrolla para el entorno laboral de la consultora de \textit{software} \textit{Colb.AI}.
\vfill
\hspace{0pt}
}
\newpage
...
Se ha uitilizado IA generativa para la asistencia en la redacción de esa memoria, principalmente para borradores iniciales, además de para reestrucurar y reescribir secciones algunas secciones. 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_cpbl_bars_Opencode_-_ConfortMental.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_cpbl_bars_Claudecode_-_ConfortMental.png}
    \caption{Test $t$ de Welch — colbPowers vs.\ baseline, ClaudeCode, Caso~1.}
    \label{fig:ttest_cpbl_cc_cm}
\end{figure}

\textit{Análisis estadístico.} El incremento de tokens atribuible a colbPowers es estadísticamente significativo en tres de las cuatro combinaciones experimento/herramienta: OpenCode ConfortMental ($p=0{,}032$), ClaudeCode ConfortMental ($p=0{,}005$) y OpenCode ClassVis ($p=0{,}019$). La excepción es ClaudeCode ClassVis ($p=0{,}077$), donde la mayor varianza entre ejecuciones reduce la potencia del test. El número de tool calls no alcanza significancia estadística en ninguna combinación para esta comparación.

\subsubsection{Métricas de calidad (Caso~2)}

Las métricas de calidad del código ---complejidad ciclomática e índice de mantenibilidad--- únicamente se calcularon para el Caso~2 (ClassificationVisualization), donde el asistente genera la arquitectura completa desde cero. En el Caso~1, al tratarse de modificaciones puntuales sobre una base de código preexistente, la atribución de estas métricas al comportamiento del asistente sería metodológicamente problemática.
...
La Figura~\ref{fig:delta_calidad} resume estas diferencias como delta (colbPowers $-$ baseline). Las Figuras~\ref{fig:ttest_cpbl_opencode_classvis} y~\ref{fig:ttest_cpbl_claudecode_classvis} presentan los valores absolutos con barras de error y brackets de significancia.
...
\begin{figure}[H]
    ...
    \caption{Test $t$ de Welch — colbPowers vs.\ baseline, OpenCode, Caso~2 (* $p<0{,}05$).}
...
\begin{figure}[H]
    ...
    \caption{Test $t$ de Welch — colbPowers vs.\ baseline, ClaudeCode, Caso~2.}
    \label{fig:ttest_cpbl_claudecode_classvis}
\end{figure}

\textit{Análisis estadístico.} La mejora de calidad de OpenCode con colbPowers es estadísticamente significativa: reducción de complejidad ciclomática ($p=0{,}014$) y aumento del índice de mantenibilidad ($p=0{,}037$). En ClaudeCode, donde la baseline ya produce código de alta calidad, los efectos son consistentes en dirección pero no alcanzan significancia estadística (complejidad: $p=0{,}508$; mantenibilidad: $p=0{,}754$).
...
\begin{figure}[H]
    ...
    \caption{Test $t$ de Welch — OpenCode vs.\ ClaudeCode, baseline, Caso~2. Los asteriscos indican significancia estadística.}
...
\begin{figure}[H]
    ...
    \caption{Test $t$ de Welch — OpenCode vs.\ ClaudeCode, colbPowers, Caso~2.}
    \label{fig:ttest_occc_colbpowers_classvis}
\end{figure}

\textit{Análisis estadístico.} En la condición baseline, las diferencias de calidad entre asistentes son estadísticamente significativas: complejidad ciclomática ($p=0{,}007$) e índice de mantenibilidad ($p=0{,}039$). Con colbPowers activo, ambas diferencias dejan de ser significativas (complejidad: $p=0{,}932$; mantenibilidad: $p=0{,}143$), confirmando la convergencia de calidad que propicia el flujo SDD. Las tablas completas del análisis estadístico se recogen en el Anexo~\ref{sec:anexo_ttest}.
...
\end{itemize}
En todos los casos se recomendaría aumentar el numero de iteraciones a a $n \geq 5$ para obtener mayor rigurosidad estadística.
...
Las siguientes tablas recogen los resultados completos del análisis estadístico mediante el test $t$ de Welch para las dos comparaciones principales del experimento. Los tamaños muestrales son $n=3$ por grupo; los resultados deben interpretarse con cautela.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{graficos/ttest_analisis/results_cpbl_table.png}
    \caption{Resultados del test $t$ de Welch: colbPowers vs.\ baseline.}
...
\begin{figure}[H]
    ...
    \caption{Resultados del test $t$ de Welch: OpenCode vs.\ ClaudeCode.}
