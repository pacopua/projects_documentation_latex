...
\begin{titlepage}
    ...
    \vspace{2cm}
    {\Large \textsc{\textbf{Gestió De Projectes - Entregable 4}}\par}
    {\large \textsc{Documento Final}\par}
...
Para validar empíricamente la calidad del código generado por el sistema, se emplearán métricas de análisis estático ampliamente adoptadas en la ingeniería del software \cite{sommerville_se_2015}. Estas métricas serán calculadas de forma automatizada por el agente \textit{Reviewer} sobre el código del repositorio local, y sus resultados se presentarán al desarrollador como parte de la auditoría. Para cada métrica, se definirá un umbral de aceptación a partir del cual el agente considerará el código como no conforme y señalará las funciones o módulos problemáticos. El conjunto de métricas aquí descrito no es definitivo; está previsto ampliarlo en iteraciones futuras del proyecto a medida que se identifiquen indicadores adicionales relevantes para el contexto del sistema.
...
\hline
Jefe del Proyecto & $\sim$77h & \textbf{GP} (25\%), \textbf{TP2} (100\%), \textbf{DA3} (19\%) \\
\hline
Investigador / Documentador & $\sim$190h & \textbf{GP} (75\%), \textbf{TP1} (100\%), \textbf{DA2} (30\%), \textbf{DA4} (25\%) \\
\hline
Programador Junior & $\sim$210h & \textbf{DA1} (100\%), \textbf{DA2} (70\%), \textbf{DA3} (81\%), \textbf{DA4} (75\%) \\
\hline
Tester & $\sim$60h & \textbf{PV1} (100\%), \textbf{PV2} (100\%) \\
\hline
\hline
\textbf{Total} & \textbf{$\sim$537h} & \\
\hline
\end{tabularx}
\caption{Distribución estimada de horas del autor por rol a lo largo del proyecto. Generación propia.}
...
\subsection{Riesgo 1: Cambios repentinos en los requerimientos por parte de la empresa}
\label{sec:riesgo1}
\begin{itemize}
...
\subsection{Riesgo 2: Bloqueo tecnológico (Vendor Lock-in) por cambio de precios o políticas en las APIs de los LLMs}
\label{sec:riesgo2}
\begin{itemize}
...
\subsection{Riesgo 3: Documentación interna insuficiente para la generación autónoma de código estructurado}
\label{sec:riesgo3}
\begin{itemize}
...
\subsection{Riesgo 4: Limitaciones de control sobre el flujo de orquestación en el ecosistema IDE}
\label{sec:riesgo4}
\begin{itemize}
...
\subsection{Riesgo 5: Desbordamiento de la ventana de contexto en proyectos grandes}
\label{sec:riesgo5}
\begin{itemize}
...
\subsection{Riesgo 6: Exceso de fricción en el bucle de desarrollo por auditorías demasiado estrictas}
\label{sec:riesgo6}
\begin{itemize}
...
\textbf{Riesgos:} El principal riesgo ambiental es que, aunque el sistema genere menos llamadas a los LLMs que las soluciones actuales, cada llamada sea inherentemente más compleja al tener que seguir un \textit{workflow} específico con mayor volumen de contexto. Esto podría traducirse en un consumo energético por interacción equivalente o superior al de las soluciones existentes, reduciendo el beneficio ambiental esperado.

\newpage

\section{Cambios de Alcance del Proyecto}
\label{sec:cambios_alcance}
\subsection{Motivación del cambio}
\label{sec:motivacion_cambio}
La planificación inicial del proyecto partía de un conocimiento superficial del ecosistema de herramientas de asistencia al desarrollo existentes. En el GEP se identificaron herramientas como Spec Kit, AgentOS o los asistentes comerciales (Copilot, Cursor), pero sin profundizar en el grado de extensibilidad que estos entornos ofrecían. Durante la fase de Trabajo Previo TP1, al investigar en detalle la arquitectura de los asistentes de código CLI más avanzados, se descubrió que el ecosistema era considerablemente más rico y maduro de lo que se había anticipado.

En concreto, herramientas como ClaudeCode y OpenCode ofrecen mecanismos nativos de extensibilidad —plugins, hooks de ciclo de vida y el protocolo MCP (Model Context Protocol)— que permiten definir flujos de trabajo de programación estructurados de forma composable, sin necesidad de construir un asistente desde cero. Lo que en la planificación inicial se concebía como un problema de ingeniería complejo (imponer un flujo Spec-First sobre un agente de código) resultó ser un problema de configuración e integración, abordable dentro del alcance de un TFG.

Este descubrimiento, unido a la evolución de las necesidades de Colb.Ai durante el período de desarrollo, fue el detonante del cambio de alcance. La empresa reconoció que el problema real al que se enfrentaba no era la ausencia de herramientas de generación de código —de eso ya había suficiente oferta—, sino la falta de un marco de trabajo que garantizase una estructura de proyecto consistente y trazable con independencia del desarrollador o del asistente concreto que se utilizase. Dicho de otro modo: a Colb.Ai no le importa cómo cada desarrollador aplica las tareas del día a día, sino que todos los proyectos arranquen con una definición clara de su arquitectura, mantengan actualizado un conjunto de documentos de referencia (la constitución del proyecto y el archivo features.md), y que cualquier nueva funcionalidad sea coherente con esas bases antes de ser implementada.

Ante este rediseño del problema, en consenso con la empresa y el director del proyecto, se tomó la decisión de redirigir el foco del TFG hacia una contribución más ajustada a las necesidades reales de Colb.Ai: diseñar, integrar y evaluar el flujo de trabajo completo que cubre el ciclo de vida de un proyecto software, desde la toma de requerimientos hasta el seguimiento del cliente, aprovechando las capacidades de extensibilidad de los asistentes existentes en lugar de construir uno nuevo.


\subsection{Nuevos objetivos del proyecto}
\label{sec:nuevos_objetivos}
El proyecto pasa a estructurarse en torno a tres pilares complementarios, cada uno de los cuales aborda una fase diferente del ciclo de vida de un proyecto de software:

\textbf{Toma de Requerimientos:} Diseñar e implementar un flujo de trabajo asistido por IA que, a partir de la transcripción de una reunión con el cliente, sea capaz de generar un plan de implementación estructurado, explorando automáticamente los frameworks y tecnologías relevantes mediante búsqueda semántica en la web.

\textbf{Spec Driven Development (Estructura del repositorio):} Integrar y configurar los asistentes de código de tipo CLI (ClaudeCode y OpenCode) con un conjunto de herramientas, plugins y workflows personalizados que impongan un flujo de trabajo Spec-First al equipo de desarrollo, garantizando que cada implementación esté precedida por una especificación validada y revisada.

\textbf{Seguimiento del cliente:} Automatizar la generación de documentación de seguimiento post-reunión, produciendo de forma estructurada el resumen de lo acordado y los próximos pasos para ambas partes, a partir de la transcripción de las reuniones periódicas con el cliente.

\subsection{Continuidad con la planificación inicial}
\label{sec:continuidad_planificacion}
Este cambio de alcance no supone un abandono de los objetivos originales del GEP, sino una evolución coherente de los mismos. Los sub-objetivos de coherencia arquitectónica, trazabilidad entre especificación y código, y modularidad del sistema se mantienen intactos y se materializan ahora a través de la configuración de los asistentes existentes en lugar de mediante el desarrollo de uno propio.

Los riesgos identificados en el GEP (vendor lock-in, gestión de la ventana de contexto, fricción en el bucle de desarrollo) se mantienen vigentes y son abordados específicamente en el diseño de los módulos descritos en los capítulos siguientes. En concreto, el riesgo de vendor lock-in se mitiga mediante la integración paralela de ClaudeCode y OpenCode, tal como se describe en el capítulo \ref{sec:sdd_implementacion}.
...
\subsection{Módulo de transcripción}
...
\subsubsection{Reuniones telemáticas}
...
\subsubsection{Reuniones presenciales}
...
\subsection{Módulo de procesamiento de requerimientos}
...
\subsubsection{Enfoque mediante Claude Cowork}
...
Para extender y configurar OpenCode de forma eficaz, es necesario comprender con exactitud en qué momento del ciclo se carga cada componente ---plugins, skills, herramientas y agentes--- y cómo interactúan durante una conversación. A continuación se describe este ciclo de forma detallada, desde la inicialización del proceso hasta la entrega de la respuesta al usuario.

\paragraph{Inicialización perezosa del espacio de trabajo.}
OpenCode implementa el patrón de inicialización perezosa (\textit{lazy initialization}) mediante el objeto \texttt{InstanceState}. Cuando el usuario abre un proyecto por primera vez en la sesión, el sistema no instancia de inmediato todos los servicios: los plugins, el registro de herramientas, el catálogo de skills y los clientes MCP se crean la primera vez que son necesarios y se mantienen en caché para el resto de la sesión, usando la librería de efectos \texttt{Effect-TS} para gestionar las dependencias entre servicios. Esto permite que el proceso arranque rápidamente con independencia del tamaño del proyecto.

\paragraph{Descubrimiento de plugins y herramientas.}
Cuando el registro de herramientas se inicializa por primera vez, OpenCode ejecuta el proceso de descubrimiento de plugins. Los plugins se cargan desde tres fuentes: (1) los plugins internos del propio asistente, (2) los paquetes npm instalados en el sistema que declaren el punto de entrada del plugin en su \texttt{package.json}, y (3) los archivos JavaScript o TypeScript localizados en los directorios \texttt{tool/} o \texttt{tools/} del proyecto. Cada plugin puede registrar herramientas adicionales mediante la interfaz de hooks que OpenCode expone, concretamente el campo \texttt{Hooks.tool}. Todas las herramientas ---las nativas del asistente más las aportadas por plugins y por los servidores MCP configurados--- se registran en el registro global y se describen al modelo mediante un esquema JSON.
...
\begin{enumerate}
    \item \textbf{Prompt base del proveedor}: instrucciones genéricas del asistente, adaptadas al modelo concreto que esté activo (Anthropic Claude, Google Gemini u otros).
...
La respuesta del modelo llega como un flujo asíncrono de eventos (\texttt{fullStream}) que el componente \texttt{SessionProcessor} consume evento a evento. Los tipos de eventos posibles incluyen: inicio de paso, fragmentos de texto (\textit{text-delta}), razonamiento interno del modelo (\textit{reasoning}), llamadas a herramienta (\textit{tool-call}), resultados de herramienta, y señales de finalización. A medida que se reciben fragmentos de texto, estos se publican en el bus de eventos interno de OpenCode, que los propaga al frontend mediante Server-Sent Events (SSE), permitiendo que la interfaz de usuario renderice la respuesta en tiempo real con baja latencia.

\paragraph{Ejecución de herramientas.}
Cuando el modelo decide invocar una herramienta, el \texttt{SessionProcessor} recibe el evento \texttt{tool-call} con el nombre de la herramienta y sus parámetros. Antes de ejecutar la herramienta, se disparan los hooks \texttt{tool.execute.before} de todos los plugins registrados; tras la ejecución, se disparan los hooks \texttt{tool.execute.after}. El resultado se añade al historial de conversación como un nuevo mensaje de tipo \textit{tool result} y se inicia un nuevo ciclo del bucle ReAct, reenviando el historial actualizado al modelo.
...
\paragraph{La herramienta TodoWrite.}
Una herramienta especialmente relevante para el flujo SDD es \texttt{TodoWrite}, que permite al modelo crear y actualizar una lista de tareas estructurada. Conviene aclarar que esta herramienta no implementa ningún mecanismo de cumplimiento a nivel de código: cuando el modelo escribe la lista de tareas, el sistema únicamente persiste las entradas en una base de datos SQLite local y publica un evento en el bus interno para que la interfaz muestre la lista actualizada. La disciplina de ejecutar las tareas en orden, marcar cada una como completada antes de avanzar a la siguiente, o respetar los estados \texttt{pending} / \texttt{in\_progress} / \texttt{completed}, es puramente una consecuencia del texto de descripción de la herramienta; el runtime no bloquea al modelo si decide no seguir ese protocolo. En la práctica, la efectividad de TodoWrite como mecanismo de planificación depende de la calidad de las instrucciones del sistema y de la capacidad de seguimiento de instrucciones del modelo utilizado.

\paragraph{Pipeline de eventos hacia la interfaz de usuario.}
Toda la actividad generada por el agente ---fragmentos de texto, llamadas a herramienta, actualizaciones de la lista de tareas--- se publica en el bus de eventos interno mediante el servicio \texttt{Bus}. Este bus propaga los eventos al frontend a través de una conexión SSE (\textit{Server-Sent Events}), donde la interfaz, construida con \textit{Solid.js}, recibe y aplica los eventos en tiempo real actualizando reactivamente la vista. El diseño asíncrono permite que la llamada HTTP que inicia la conversación retorne de inmediato (respuesta 202) mientras toda la actividad del agente fluye de forma independiente por el canal SSE.

\paragraph{Similitud con ClaudeCode.}
ClaudeCode implementa un pipeline arquitectónicamente muy similar. Aunque las implementaciones concretas difieren ---ClaudeCode no es software libre---, el flujo conceptual es idéntico: inicialización perezosa de servicios al inicio de la sesión, construcción del system prompt a partir de archivos de instrucciones y catálogos de skills, bucle ReAct con llamadas a herramienta y streaming de tokens, carga perezosa de skills bajo demanda, y mecanismos de hooks para interceptar el ciclo de vida. Esta equivalencia es precisamente la que hace posible que un plugin como colbPowers, que se apoya en skills y hooks genéricos, funcione de forma transparente en ambos asistentes con mínimas diferencias de integración.
...
\textbf{Hook:} Un hook es un callback que se ejecuta automáticamente en momentos específicos del ciclo de vida del agente: al iniciar una sesión (SessionStart), antes o después de editar un archivo, al ejecutar un comando de terminal, etc. Los hooks permiten interceptar el flujo de trabajo del agente e inyectar comportamiento sin que el usuario lo solicite explícitamente. En colbPowers, el hook SessionStart es el mecanismo que inyecta el skill de bootstrap en cada sesión.

\textbf{Skill:} Una skill es un documento Markdown que contiene instrucciones especializadas para el agente sobre cómo comportarse en una tarea concreta. A diferencia de un prompt de usuario, una skill forma parte del contexto del sistema y está disponible para el agente como conocimiento procedimental persistente. Cuando el agente necesita ejecutar una tarea para la que existe una skill (por ejemplo, hacer brainstorming de una feature o escribir un plan), lee ese documento y sigue sus instrucciones.

\textbf{Agente (subagente):} En el contexto de estos asistentes, un agente o subagente es una instancia del modelo de lenguaje creada dinámicamente por el orquestador para ejecutar una tarea concreta en aislamiento. A diferencia del agente principal, el subagente arranca con un contexto específico y sin acceso al historial de conversación del orquestador. Esta separación garantiza que cada tarea se ejecuta con el contexto mínimo necesario, evitando interferencias entre tareas y reduciendo el consumo de tokens.

\textbf{Plugin:} Un plugin es un paquete instalable que agrupa y distribuye extensiones para el asistente: skills, hooks, comandos y agentes. Su función es análoga a la de un paquete npm: encapsula un conjunto coherente de funcionalidades y las hace instalables en cualquier proyecto con un único comando. Los plugins se instalan a nivel de usuario o de proyecto y son la unidad de distribución del ecosistema.

\textbf{MCP (Model Context Protocol):} El MCP es un protocolo estándar abierto que define cómo un proceso externo puede exponer herramientas al agente. Un servidor MCP es un proceso independiente (escrito en cualquier lenguaje) que anuncia un conjunto de herramientas invocables mediante el protocolo. Cuando el agente necesita una capacidad externa ---consultar un grafo de código, buscar en la web, leer una base de datos---, el runtime llama al servidor MCP correspondiente, que ejecuta la operación y devuelve el resultado. La separación entre el runtime del agente y el servidor MCP garantiza que cualquier herramienta externa puede integrarse sin modificar el asistente.

\subsubsection{Archivo de instrucciones del proyecto}
\label{sec:archivo_instrucciones}
Ambos asistentes se configuran a nivel de proyecto mediante archivos de instrucciones persistentes que actúan como contexto estático del agente. ClaudeCode utiliza un archivo denominado CLAUDE.md ubicado en la raíz del repositorio; OpenCode utiliza un archivo equivalente en su directorio de configuración. En cada sesión de trabajo, el asistente incorpora automáticamente este archivo en su system prompt, lo que garantiza acceso inmediato a las reglas del proyecto, el stack tecnológico y los estándares de código sin que el desarrollador los proporcione manualmente. El contenido típico incluye la descripción del proyecto, la pila tecnológica, los estándares de código del equipo, las herramientas disponibles y las restricciones que el agente debe respetar.

\subsubsection{Plugins y MCPs}
\label{sec:plugins_mcps}
Ambos asistentes son extensibles mediante el protocolo MCP. La instalación y configuración de plugins se realiza a nivel de proyecto o de usuario, garantizando que el conjunto de herramientas disponibles sea consistente en todas las sesiones. Cada plugin puede registrar servidores MCP adicionales que amplían el esquema de herramientas del agente; el agente ve estas herramientas adicionales exactamente igual que las nativas (Read, Write, Bash), y puede invocarlas mediante el mismo mecanismo de tool call descrito en la sección~\ref{sec:react_pattern}.

\subsubsection{Hooks de ciclo de vida}
\label{sec:hooks_ciclo_vida}
ClaudeCode expone un sistema de hooks configurables en un archivo hooks.json que permite registrar callbacks para distintos eventos del ciclo de vida del agente. Los hooks más relevantes para el flujo SDD son SessionStart (inyección de contexto al inicio de sesión) y PreToolUse (intercepción antes de ejecutar una herramienta concreta, útil para forzar consultas al grafo de código antes de cualquier modificación). En OpenCode, estos hooks se implementan mediante callbacks definidos en el plugin JavaScript correspondiente, con semántica equivalente.
...
La integración de esta herramienta aborda directamente el problema de gestión de la ventana de contexto identificado en el apartado de riesgos (sección~\ref{sec:riesgo5}). En pruebas preliminares realizadas sobre un monorepo de Colb.Ai, el agente con code-review-graph integrado necesitó aproximadamente 40.000 tokens para describir la arquitectura del proyecto, frente a los 64.000 tokens requeridos sin la herramienta, lo que supone una reducción del 37,5\verb|%| en esa prueba concreta. Las capacidades que expone al agente incluyen: análisis de diffs de git para identificar cambios de mayor riesgo, cálculo del radio de impacto (\textit{blast radius}) de cualquier modificación, identificación de los flujos de ejecución afectados, búsqueda semántica de entidades de código, y obtención de una visión de alto nivel de la arquitectura del sistema.
...
colbPowers \cite{superpowers_2025} es un plugin instalable en cualquier proyecto que implementa el flujo de trabajo Spec-First de forma autónoma. Una vez instalado, no requiere que el desarrollador recuerde invocar comandos ni seguir un protocolo manual: el propio agente detecta el estado del proyecto al inicio de cada sesión y guía al usuario por el flujo correcto de forma proactiva.
...
Con el objetivo de evaluar empíricamente el impacto de las herramientas desarrolladas, se ha diseñado un experimento controlado que compara tres condiciones de implementación sobre dos casos de uso distintos. Los experimentos están planificados y pendientes de ejecución; esta sección describe el diseño experimental y la estructura de resultados que se completará con los datos obtenidos.

\subsubsection{Diseño experimental}
\label{sec:diseno_experimental}
El experimento sigue un diseño de tres condiciones (A/B/C) con tres repeticiones por condición (n=3), ejecutadas con temperatura 0 para minimizar la varianza estocástica del modelo. Las tres condiciones son:

\textbf{Condición A --- Sin plugins:} El agente dispone únicamente de las herramientas nativas del asistente CLI (Read, Write, Edit, Bash, Grep). No se activa ningún plugin ni servidor MCP adicional.

\textbf{Condición B --- Con code-review-graph:} El agente dispone de las herramientas nativas más el servidor MCP de code-review-graph. El agente puede consultar el grafo de dependencias, calcular el blast radius de los cambios y realizar búsquedas semánticas sobre el código.

\textbf{Condición C --- Con colbPowers + code-review-graph:} El agente opera bajo el flujo SDD completo de colbPowers, con acceso además al servidor MCP de code-review-graph. La tarea se ejecuta siguiendo el ciclo constitution $\rightarrow$ features $\rightarrow$ spec $\rightarrow$ plan $\rightarrow$ execute $\rightarrow$ review.

Las métricas recogidas en cada ejecución son: tokens de entrada consumidos, tokens de salida consumidos, número total de tool calls, número de tool calls específicas de code-review-graph, cobertura de la implementación (porcentaje de ítems del enunciado correctamente implementados), y número de regresiones introducidas (cambios en funcionalidad no solicitada).

\subsubsection{Caso 1: ConfortMental --- cambios cross-cutting en base de código real}
\label{sec:caso1_confortmental}
El primer caso de uso aplica el experimento sobre un repositorio de producción de Colb.Ai: la aplicación móvil ConfortMental, desarrollada en React Native. La tarea consiste en implementar los cinco cambios globales del documento de revisión de la aplicación (Bloque 1): unificación tipográfica con DM Sans y DM Serif Display, corrección de contraste de textos secundarios a WCAG AA, sustitución del patrón de validación de formularios por mensajes inline en rojo, humanización y traducción de mensajes de error del sistema, y unificación de las etiquetas de frecuencia de check-in.

Este caso de uso es especialmente adecuado para evaluar code-review-graph porque los cinco cambios son cross-cutting: afectan a múltiples pantallas y componentes de forma transversal. La hipótesis es que el agente bajo la condición B identificará más instancias de cada patrón (mayor cobertura) y consumirá menos tokens de exploración que el agente bajo la condición A, que debe descubrir la estructura del proyecto mediante herramientas estándar.

\begin{table}[H]
\centering
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Métrica} & \textbf{Cond. A} & \textbf{Cond. B} & \textbf{Cond. C} \\
 & \textbf{(sin plugins)} & \textbf{(code-review-graph)} & \textbf{(colbPowers + CRG)} \\
\hline
Tokens entrada (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tokens salida (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls totales (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls CRG (media) & N/A & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Cobertura (media \verb|%|) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
\hline
\end{tabular}
\caption{Resultados Caso 1: ConfortMental}
\label{tab:caso1_resultados}
\end{table}

\subsubsection{Caso 2: Aplicación de simulación 3D}
\label{sec:caso2_simulacion3d}
El segundo caso de uso aplica el experimento sobre una tarea de nueva implementación desde cero: el desarrollo de una aplicación de simulación 3D en la que, al pulsar una tecla, aparece una esfera en el entorno tridimensional; al hacer clic sobre ella, la esfera explota en una animación de partículas. A diferencia del caso ConfortMental (modificación de una base de código existente), este caso evalúa el comportamiento de las herramientas en la creación de funcionalidad nueva.

Este caso es especialmente relevante para evaluar colbPowers porque la condición C puede ejecutar el ciclo completo de la metodología SDD: el Architect define la constitution del proyecto y las features, el Developer genera el documento de diseño y el plan de tareas, y la ejecución se realiza tarea a tarea con revisión al final. La hipótesis es que la condición C produce código de mayor calidad estructural (menor complejidad ciclomática, mejor índice de mantenibilidad) y mayor trazabilidad entre las decisiones de diseño y el código implementado.
...
Tokens salida (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Tool calls totales (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Cobertura funcional (media \verb|%|) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Complejidad ciclomática (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
Índice de mantenibilidad (media) & {[}PENDIENTE{]} & {[}PENDIENTE{]} & {[}PENDIENTE{]} \\
\hline
\end{tabular}
\caption{Resultados Caso 2: Aplicación de simulación 3D}
\label{tab:caso2_resultados}
\end{table}

\newpage

\section{Seguimiento del Cliente}
...
\subsection{Generación de documentación con Claude Cowork}
\label{sec:generacion_doc_claude_cowork}
El flujo implementado mediante Claude Cowork se integra directamente con el proceso de reunión. Al finalizar cada reunión de seguimiento con el cliente, la transcripción o el resumen de lo acordado se proporciona como entrada a una skill de Claude Cowork desarrollada específicamente para este propósito. La skill procesa la información y genera de forma automática un documento Word estructurado que incluye los siguientes apartados: el resumen de los temas tratados en la reunión, las decisiones tomadas y los acuerdos alcanzados, las tareas asignadas al equipo de Colb.Ai con sus plazos correspondientes, y las tareas o acciones que corresponden al cliente. Este documento se genera y está disponible para ser enviado al cliente en el mismo momento en que finaliza la reunión, eliminando la latencia habitual en la producción de este tipo de documentación.
...
\subsection{Alternativa de código abierto}
...
\label{sec:estado_implementacion}
Este capítulo describe el estado de desarrollo de cada uno de los tres módulos del proyecto en el momento de redacción de esta memoria. El objetivo es ofrecer una fotografía honesta del grado de madurez de cada componente, distinguiendo entre lo que está operativo, lo que está en proceso iterativo y lo que queda pendiente como trabajo futuro.

\subsection{Toma de Requerimientos}
\label{sec:estado_toma_requerimientos}

\subsubsection{Módulo de transcripción}
\label{sec:estado_modulo_transcripcion}
El módulo de transcripción para reuniones presenciales, basado en Azure Speech Service con diarización, está implementado e integrado en la aplicación. La transcripción automática para reuniones telemáticas no requiere implementación propia, dado que se delega en las herramientas nativas de las plataformas de videoconferencia (Microsoft Teams, Google Meet, Zoom).

\subsubsection{Implementación propia (aplicación Python)}
\label{sec:estado_implementacion_python}
La aplicación Python de toma de requerimientos está en un estado avanzado de desarrollo. El pipeline completo está implementado y ha sido validado sobre un proyecto real de Colb.Ai con resultados satisfactorios: el sistema generó un plan de implementación estructurado con propuesta tecnológica justificada y lista de issues de GitHub accionables. Está pendiente una revisión final del código para consolidar casos límite y asegurar la robustez del módulo antes de considerarlo completamente terminado.

La skill de Claude Cowork para toma de requerimientos está igualmente operativa y ha sido utilizada en proyectos de la empresa.

\textbf{Estado:} Funcional. Pendiente de revisión y validación final.

\subsection{Spec Driven Development: colbPowers}
\label{sec:estado_colbpowers}
colbPowers está en un estado funcional. La infraestructura central del plugin está completa y operativa:

\begin{itemize}
    \item El hook SessionStart inyecta correctamente el skill using-colbpowers al inicio de cada sesión, activando el comportamiento de orquestador en el agente principal.
    \item El conjunto de skills heredadas de Superpowers \cite{superpowers_2025} está disponible y funciona con la configuración de artefactos de colbSpec.
    \item El plugin ha sido instalado y probado en proyectos reales de Colb.Ai, refinándose de forma iterativa a partir del feedback de uso.
\end{itemize}

Las siguientes funcionalidades están pendientes de implementación:

\textbf{Métricas de calidad de código:} La integración de las métricas de Complejidad Ciclomática, Volumen de Halstead e Índice de Mantenibilidad en el flujo de revisión no está aún implementada. Requiere extender code-review-graph con los cálculos correspondientes sobre el AST de Tree-Sitter y exponer los resultados al agente Reviewer mediante una nueva herramienta MCP.

\textbf{Adaptación completa de rutas de artefactos:} Los skills brainstorming y writing-plans generan actualmente los documentos en la ruta heredada de Superpowers. La adaptación para que los artefactos aterricen directamente en \texttt{.specs/memory/docs/specs/} y \texttt{.specs/memory/docs/plans/} está pendiente como segunda iteración del plugin.

\textbf{Estado:} Funcional. En proceso iterativo activo. Métricas de calidad y adaptación de rutas de artefactos pendientes.

\subsection{Seguimiento del Cliente}
\label{sec:estado_seguimiento_cliente}

\subsubsection{Skill de Claude Cowork}
\label{sec:estado_skill_claude_cowork}
La skill de Claude Cowork para la generación automática de documentación de seguimiento post-reunión está implementada y en uso activo. Al finalizar cada reunión con el cliente, la skill procesa la transcripción y genera un documento Word estructurado con el resumen de los temas tratados, las decisiones acordadas y los próximos pasos para ambas partes. La skill ha sido utilizada en reuniones reales de Colb.Ai con resultado satisfactorio.

\subsubsection{Alternativa propia}
\label{sec:estado_alternativa_propia}
La implementación propia basada en python-docx (descrita en la sección~\ref{sec:alternativa_codigo_abierto}) está pendiente de desarrollo. En el período cubierto por esta memoria, la skill de Claude Cowork ha cubierto completamente la necesidad operativa, por lo que la alternativa propia queda registrada como trabajo futuro.

\textbf{Estado:} Skill de Claude Cowork operativa. Alternativa propia pendiente de implementación.
