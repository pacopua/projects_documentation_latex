...
Se van a mostrar los requisitos funcionales y no funcionales definidos con anteriorridad en \ref{sec:req_funcionales} y \ref{sec:req_no_funcionales}, se iterará sobre cada uno de estos justificando si se ha conseguido cubrir adecuadamente.
...
Este requisito se implementa en la fase de planificación del flujo SDD, donde el agente orquestador genera un plan de tareas granulares a partir de una funcionalidad de alto nivel definida en \texttt{features.md} antes de delegar la implementación a los subagentes especializados. Este mecanismo ha sido validado en los dos casos experimentales del Capítulo~7.
...
El plugin colbPowers ha sido validado experimentalmente sobre los dos asistentes CLI objetivo: ClaudeCode y OpenCode. La integración se realiza mediante los mecanismos nativos de cada plataforma: \texttt{CLAUDE.md} y \textit{skills} para ClaudeCode, y el directorio \texttt{.opencode/} con su sistema de reglas para OpenCode. Los experimentos del Capítulo~7 confirman la compatibilidad efectiva con ambos asistentes.
...
\begin{figure}[H]
    ...
    \caption{Análisis estadístico: colbPowers vs.\ baseline. Guiarse por el $p$ value para determinar si hay efecto, se incluye la $g$ de Hedges como métrica principal del tamaño de efecto.}
...
\begin{figure}[H]
    ...
    \caption{Análisis estadístico: OpenCode vs.\ ClaudeCode. Guiarse por el $p$ value para determinar si hay efecto, se incluye la $g$ de Hedges como métrica principal del tamaño de efecto.}
