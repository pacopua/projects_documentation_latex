...
\textbf{Hook:} Un hook es un callback que se ejecuta automáticamente en momentos específicos del ciclo de vida del agente: al iniciar una sesión (SessionStart), antes o después de editar un archivo, al ejecutar un comando de terminal, etc. Los hooks permiten interceptar el flujo de trabajo del agente e inyectar comportamiento sin que el usuario lo solicite explícitamente. En colbPowers, el hook SessionStart es el mecanismo que inyecta el skill de bootstrap en cada sesión: al arrancar, el hook ejecuta un script que lee el archivo \texttt{SKILL.md} de la skill \textit{using-colbPowers}, formatea su contenido como JSON y lo entrega al runtime como contexto adicional (\texttt{additionalContext}), que el asistente añade al system prompt antes de que el modelo reciba el primer mensaje del usuario.

\textbf{Skill:} Muy similar a una skill en Claude Cowork denifida anteriormente \ref{fig:skill_definition}, una skill es un documento Markdown que contiene instrucciones especializadas para el agente sobre cómo comportarse en una tarea concreta, y que puede añadir scripts, comandos y templates a los que referenciar. A diferencia de un prompt de usuario, una skill forma parte del contexto del sistema y está disponible para el agente como conocimiento procedimental persistente. Cuando el agente necesita ejecutar una tarea para la que existe una skill (por ejemplo, hacer brainstorming de una feature o escribir un plan), lee ese documento y sigue sus instrucciones.

\textbf{Agente (subagente):} En el contexto de estos asistentes, un agente o subagente es una instancia del modelo de lenguaje creada dinámicamente por el orquestador para ejecutar una tarea concreta en aislamiento. A diferencia del agente principal, el subagente arranca con un contexto específico y sin acceso al historial de conversación del orquestador. Esta separación garantiza que cada tarea se ejecuta con el contexto mínimo necesario, evitando interferencias entre tareas. 

\textbf{Plugin:} Un plugin es un paquete instalable que agrupa y distribuye extensiones para el asistente: skills, hooks, comandos y agentes. Su función es análoga a la de un paquete npm: encapsula un conjunto coherente de funcionalidades y las hace instalables en cualquier proyecto con un único comando. Los plugins se instalan a nivel de usuario o de proyecto y son la unidad de distribución del ecosistema. La definición de un plugin consiste en una estructura de carpetas en la que se define una carpeta \textit{skills/} con \textit{skills} definidas, otra carpeta \textit{agents/} con agentes definidos mediante lenguaje natural en archivos \textit{Markdown} para que el agente orquestrador los pueda invocar, y finalmente la definición de hooks en un \textit{hooks.json}.
...
% TODO: Revisar la distinción entre MCP y Skills
\textbf{Distinción entre MCP y Skill:} Los conceptos de MCP y Skill pueden parecer similares en el papel, puesto que ambos basicamente le proporcionan al agente de programacion una herramienta a la que puede llamar. La principal distinción entre los dos conceptos, es el hecho de que una skill, suele ser un prompt dentro de el cual se pueden llamar herramientas, las cuales se ejecutan de forma local. A diferencia de las herramientas que nos proporcionan un MCP, que aunque tienen una descripción explicando cúando deben ser utilizadas, no incluyen un prompt de instrucciones para el agente, además de que la ejecución de la herramienta proporcionada por el MCP, se suele hacer en el servidor de ese dado MCP, mientras que las Skills de ejecutan de forma local principalmente.
...
Sin embargo, existen diferencias arquitectónicas significativas que conviene señalar. En lo que respecta a los proveedores de LLM, ClaudeCode es exclusivo de Anthropic y sus modelos Claude, mientras que OpenCode soporta más de veinticinco proveedores a través del SDK de Vercel AI (OpenAI, Google, Azure, Bedrock, Mistral, Groq, GitHub Copilot, entre otros). En cuanto al modelo de interfaz, ClaudeCode es exclusivamente una herramienta de línea de comandos cuya interfaz se renderiza en el terminal mediante React e Ink, mientras que OpenCode expone un servidor HTTP local (Hono) que sirve una aplicación web SolidJS y soporta adicionalmente aplicaciones de escritorio nativas (Tauri y Electron). En materia de persistencia, ClaudeCode mantiene el estado de la conversación en memoria y en ficheros JSON locales, mientras que OpenCode persiste las sesiones en una base de datos SQLite con instantáneas del sistema de ficheros antes de cada invocación de herramienta, lo que permite revertir cambios. Finalmente, ClaudeCode utiliza TypeScript estándar para la composición de servicios, mientras que OpenCode hace un uso intensivo del framework Effect (programación funcional monádica) para la gestión de dependencias, errores y concurrencia.
...
\textbf{Condición C --- Con colbPowers:} El agente opera bajo un flujo de SSD de colbPowers, sin embargo no tiene acceso a herramientas de expliración de códico proporcionadas por code-review-graph.

\textbf{Condición D --- Con colbPowers + code-review-graph:} El agente opera bajo el flujo SDD completo de colbPowers, con acceso además al servidor MCP de code-review-graph. La tarea se ejecuta siguiendo el ciclo constitution $\rightarrow$ features $\rightarrow$ spec $\rightarrow$ plan $\rightarrow$ execute $\rightarrow$ review.
...
\paragraph{Metodología:} La metodología del expoe
...
\textbf{Estado:} Funcional. En proceso iterativo activo. Métricas de calidad pendientes.
