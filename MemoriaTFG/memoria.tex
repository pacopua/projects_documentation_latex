...
Finalmente, nótese que el Caso 1 no incluye métricas de calidad de código. Al tratarse de un proyecto ya iniciado con base preexistente, medir la complejidad ciclomática o el índice de mantenibilidad de cambios puntuales en archivos existentes resultaría metodológicamente problemático. El Caso 1 evalúa el comportamiento del sistema sobre un repositorio real en producción, aportando evidencia sobre el overhead de tokens y el uso del grafo de dependencias en tareas de modificación incremental. Las métricas de calidad de código se reservan para el Caso 2, donde el agente es responsable de la arquitectura completa, permitiendo una comparación más limpia entre condiciones. 
...
\textit{Análisis estadístico.} En la condición baseline, las diferencias de calidad entre asistentes presentan un tamaño de efecto grande (véase Figura~\ref{fig:ttest_occc_baseline_classvis}), con $p$-values indicativos de $p=0{,}007$ para complejidad ciclomática y $p=0{,}039$ para índice de mantenibilidad. Con colbPowers activo, la magnitud de las diferencias se reduce sustancialmente: la $d$ de Cohen cae a valores próximos a cero en ambas métricas, confirmando la convergencia de calidad que propicia el flujo SDD independientemente del asistente. Las tablas completas del análisis estadístico se recogen en el Anexo~\ref{sec:anexo_ttest}.
